...
"""
Build focused POLDIS GAMMA references with callback-level P2B component dumps.
...
The helper:
  ...
  * parses the callback-level POLDIS_COMPONENT summary lines
...
DEFAULT_TAG = "plain58-gamma-poldis-components"
...
COMPONENT_RE = re.compile(
    r"^\s*POLDIS_COMPONENT\s*"
    r"(?P<label>[A-Za-z0-9_.-]+)\s+"
    r"LO=\s*(?P<lo>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"LOerr=\s*(?P<loerr>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"NLO=\s*(?P<nlo>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"NLOerr=\s*(?P<nloerr>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"dNLO=\s*(?P<dnlo>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"NNLO=\s*(?P<nnlo>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"NNLOerr=\s*(?P<nnloerr>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"dNNLO=\s*(?P<dnnlo>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s*$"
)
...
def parse_component_summaries(text: str) -> Dict[str, dict]:
    ...
    for line in text.splitlines():
        ...
        components[label] = {
            "LO": measurement_payload(
                Measurement(float(match.group("lo")), float(match.group("loerr")))
            ),
            "NLO": measurement_payload(
                Measurement(float(match.group("nlo")), float(match.group("nloerr")))
            ),
            "NNLO": measurement_payload(
                Measurement(float(match.group("nnlo")), float(match.group("nnloerr")))
            ),
            "dNLO_pb": float(match.group("dnlo")),
            "dNNLO_pb": float(match.group("dnnlo")),
        }
    return components
...
def build_reference_payload(
    ...
    payload = {
        "tag": tag,
        "mode": "poldis-gamma-component-reference",
...
def render_reference_text(payload: dict) -> str:
    ...
    lines = [
        "POLDIS GAMMA P2B component reference",
        "====================================",
        f"Tag: {payload['tag']}",
        ...
        "Note",
        "----",
        "The callback decomposition comes from the P2B user-driver stream.",
        "At NLO this exposes the Born-projected seed, accepted real tree, and P2B counterevent pieces.",
        "",
        "Unpolarized totals",
        "------------------",
    ...
    polarized_components = payload["runs"]["polarized"].get("components", {})
    if polarized_components:
        lines.extend(
            [
                "",
                "Polarized Callback Components",
                "-----------------------------",
            ]
        )
        for label, component in polarized_components.items():
            if (
                abs(component["LO"]["value_pb"])
                + abs(component["NLO"]["value_pb"])
                + abs(component["NNLO"]["value_pb"])
                <= 1e-30
            ):
                continue
            lines.append(
                f"{label}: "
                f"LO {fmt_measurement(component['LO'])}, "
                f"NLO {fmt_measurement(component['NLO'])}, "
                f"dNLO {component['dNLO_pb']:.10f} pb"
            )
        ...
        if parallel_variants:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, min(args.jobs, len(variants)))) as executor:
                ...
                for future in concurrent.futures.as_completed(future_map):
                    ...
                    print(
                        f"[progress:poldis-gamma-component-reference] "
