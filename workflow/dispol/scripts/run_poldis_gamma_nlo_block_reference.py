...
"""
Build focused POLDIS GAMMA references with deeper polarized NLO block dumps.

The helper:
  * templates user_dijet_rivetplots_p2b_components.f into a campaign-local
    work area
  * compiles a local poldis.x from the parallel poldis_gamma_nlo_blocks.f
    core in separate unpolarized/polarized subdirectories
  * runs the two jobs
  * parses the printed LO/NLO/NNLO inclusive totals
  * parses the polarized POLDIS_NLO_BLOCK summary lines
...
DEFAULT_TAG = "plain59-gamma-poldis-nlo-blocks"
...
NLO_BLOCK_RE = re.compile(
    r"^\s*POLDIS_NLO_BLOCK\s+"
    r"label=(?P<label>[A-Za-z0-9_.-]+)\s+"
    r"value=\s*(?P<value>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s+"
    r"error=\s*(?P<error>[+-]?\d+(?:\.\d*)?(?:[Ee][+-]?\d+)?)\s*$"
)

RUNTIME_FILES = ("poldis_gamma_nlo_blocks.f", "gbook.f", "jetalg.f")
...
def parse_nlo_block_summaries(text: str) -> Dict[str, dict]:
    blocks: Dict[str, dict] = {}
    for line in text.splitlines():
        match = NLO_BLOCK_RE.match(line.strip())
        if not match:
            continue
...
def compile_command(poldis_dir: Path) -> list[str]:
    compile_script = poldis_dir / "compile_dijet_rivetplots_gamma_nlo_blocks"
...
def patch_user_source(
    ...
    if not seen_unpolarized_pdf or not seen_polarized_pdf:
        raise RuntimeError(
            "Failed to patch POLDIS PDF set lines in "
            "user_dijet_rivetplots_p2b_components.f"
        )
    rendered = "\n".join(patched)
...
def expected_top_name(ipol: int) -> str:
    return "dijets_unpol_GAM_p2bc.top" if ipol == 0 else "dijets_pol_GAM_p2bc.top"
...
def ensure_compiled(poldis_dir: Path, run_dir: Path, dry_run: bool) -> None:
    ...
    if not dry_run and binary.exists():
        source_mtime = max(
            (run_dir / filename).stat().st_mtime
...
def load_variant_payload(run_dir: Path, ipol: int) -> dict:
    ...
    nlo_blocks = parse_nlo_block_summaries(log_text)
...
def build_reference_payload(
    ...
    payload = {
        "tag": tag,
        "mode": "poldis-gamma-nlo-block-reference",
...
def render_reference_text(payload: dict) -> str:
    ...
    lines = [
        "POLDIS GAMMA polarized NLO block reference",
        "==========================================",
        f"Tag: {payload['tag']}",
        ...
        "Note",
        "----",
        "The polarized NLO block dump comes from the parallel poldis_gamma_nlo_blocks core.",
        "The reported blocks are accumulated inside the polarized BORNPROJ NLO coefficient-function assembly.",
        "",
        "Unpolarized totals",
        "------------------",
    ...
    polarized_blocks = payload["runs"]["polarized"].get("nlo_blocks", {})
    if polarized_blocks:
        lines.extend(
            [
                "",
                "Polarized NLO Blocks",
                "--------------------",
            ]
        )
        preferred_order = (
        ...
        for label in preferred_order:
            block = polarized_blocks.get(label)
            if block is None:
                continue
            seen.add(label)
            lines.append(f"{label}: {fmt_measurement(block)}")
        for label, block in polarized_blocks.items():
            if label in seen:
                continue
            lines.append(f"{label}: {fmt_measurement(block)}")
        reconstruction = None
        target = polarized_blocks.get("NLO_pol_pb")
        if (
            target is not None
            and "Yminus_times_G2_pb" in polarized_blocks
            and "Yplus_times_G4_pb" in polarized_blocks
            and "minusY2_times_GL_pb" in polarized_blocks
        ):
            reconstruction = (
                polarized_blocks["Yminus_times_G2_pb"]["value_pb"]
                + polarized_blocks["Yplus_times_G4_pb"]["value_pb"]
                + polarized_blocks["minusY2_times_GL_pb"]["value_pb"]
            )
            lines.extend(
                [
                    "",
                    "NLO Block Reconstruction",
                    "------------------------",
                    f"Y-weighted sum: {reconstruction:.10f} pb",
                    f"NLO_pol_pb:    {target['value_pb']:.10f} pb",
                    f"Difference:    {reconstruction - target['value_pb']:.10e} pb",
                ]
            )
        ...
        if parallel_variants:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, min(args.jobs, len(variants)))) as executor:
                ...
                for future in concurrent.futures.as_completed(future_map):
                    ...
                    print(
                        f"[progress:poldis-gamma-nlo-block-reference] "
                        f"completed {label} ({completed}/2)",
                        flush=True,
                    )
        else:
