...
Within this broader programme, general-purpose Monte Carlo event generators provide the framework in which perturbative calculations can be combined with parton showers, hadronisation, and decays to produce realistic event simulations. Among these, \Herwig\ combines perturbative hard-scattering calculations, parton showers, hadronisation, and particle decays within a unified simulation framework~\cite{Bahr:2008pv,Gieseke:2011na,Arnold:2012fq,Bellm:2013hwb,Bellm:2015jjp,Bellm:2017bvx,Bellm:2019zci,Bewick:2023tfi,Bellm:2025pcw}, and already incorporates matching to higher-order corrections as well as a dedicated spin-correlation algorithm for propagating polarization information through showering and decays~\cite{Richardson:2018pvo}. An NLO treatment of unpolarized DIS was implemented in an earlier version of the code~\cite{DErrico:2011wfa}. However, a corresponding polarized implementation within \Herwig, together with a mechanism for propagating the spin information of the realised real-emission configuration to the shower, has not been available until now.
...
In terms of these distributions and the polarizations of the hadron in the $x,y,z$ directions,\footnote{Assuming the hadron is moving in the $z$ direction.} $P_{x,y,z}$,
...
For unpolarized hadrons $H_{i\tau\tau'}(x,Q^2)\to 1/s_i\,\delta_{\tau\tau'}$, where $s_i$ is the number of spin states for parton $i$, and
Eq.~\eqref{eqn:dsigma} reduces to the standard cross section formula.

The spin-density matrix $H_{i\tau\tau'}$ defined here enters directly in the parton shower evolution discussed in the next subsection, and its generalisation to the NLO real-emission state is the subject of Section~\ref{sec:polarizeddis}.
...
Since $H_{g\pm\mp}=0$ for spin-$\frac12$ hadrons, the off-diagonal terms do not contribute in the present context but are retained for generality, e.g.\ for future application to polarized resolved-photon processes.
\begin{table}
  \begin{center}
    \begin{tabular}{|c|c|}
      \hline
...
using only the $1+B$ distribution that applies in the unpolarized case.
...
The formalism developed in this section applies to shower emissions that evolve from a Born-level hard process. Its extension to the case in which the shower is instead seeded by a realised NLO real-emission configuration is described in Section~\ref{sec:polarizeddis}.
...
\end{align}
with $x_1 \leq -1$ and $x_2 \leq 1$, where the equalities are reached at the Born endpoint; in the real-emission region one has $x_1<-1$.
...
\end{align}
where the inelasticity $y_B=\ldot{p_1}{q}/\ldot{p_1}{q_1}$ gives $\ell = 2/y_B -1$, and $\ell Q/2$ is the energy of the incoming lepton in the Breit frame. Defining $\cos\theta_2 = x_2 / \sqrt{x_2^2 + x_\perp^2}$, we then obtain
...
\end{align}
As a consistency check, the unpolarized case is recovered by setting $P_\ell = P_q = 0$, giving $\mathcal{A}^{NC}_{0,0}=\mathcal{A}$ as in Eq.~\eqref{eq:Aunpol}. The pure photon contribution follows from setting $C_{A,\ell}, C_{A,q}, C_{V,\ell}, C_{V,q}=0$, which yields $\mathcal{A}^{\gamma}_{P_\ell, P_q} = 2 P_\ell P_q$. For the charged-current process, $\mathcal{A}^{CC}_{P_\ell, P_q} = 2$ as an analysing-power ratio, reflecting the purely left-handed coupling of the $W^\pm$; the explicit beam and parton polarizations still enter through the overall Born normalization, which vanishes for wrong-chirality configurations.
...
The final expression for $R_2$ is as for the QCD Compton case, but with the substitution $P_q \rightarrow P_g$ in the polarized analysing power.  This follows from the CALKUL decomposition~\cite{DeCausmaecker:1981jtq}: for each gluon helicity $\lambda_g$, chirality conservation along the massless quark propagator selects a unique quark helicity at the electroweak vertex, and QCD parity invariance ensures that the two gluon helicities contribute with equal weight. The net effect is that the effective quark polarization at the DIS vertex equals the gluon polarization, $P_q^{\mathrm{eff}}=P_g$.
...
We first define the \emph{unpolarized} modified luminosity.  For an incoming quark flavour $q$,
\begin{align}
f^{\,q}_m(x_B,\mu_F)
...
\label{eq:fm-pol}
\end{align}
The kernels appearing in the unpolarized luminosity coincide with those used in the standard (unpolarized) DIS implementation and are given explicitly by
\begin{align}
...
For the polarized component of the cross section, the collinear remnants require the difference PDFs $\Delta f_i$ and the polarized splitting function $\Delta P_{gq}$.  We define a polarized modified luminosity $\Delta f_m$ with the same structure as Eq.~\eqref{eq:fm-pol} but with $f_i \to \Delta f_i$ and $A \to \Delta A$:
\begin{align}
...
\label{eq:DA-kernel}
\end{equation}
This encodes the polarized splitting function $\Delta P_{gq}(z) = 2z - 1$ (with associated finite part $2(1{-}z)$) in place of the unpolarized $P_{gq}(z)=z^2+(1{-}z)^2$ (with finite part $2z(1{-}z)$).
The quark kernels $B$ and $C$ are identical for the polarized and unpolarized luminosities, since $\Delta P_{qq}(z) = P_{qq}(z)$ in the $\overline{\text{MS}}$ scheme with the polarized finite renormalisation adopted here.

\paragraph{Decomposition of the collinear contribution.}
In the Monte Carlo implementation, the Born cross section for polarized beams at fixed lepton kinematics $(\ell \equiv 2/y_B - 1)$ has the structure
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:realg-pol}
\end{equation}
The polarized kernel~\eqref{eq:realg-pol} is proportional to $\Delta P_{gq}(x_p)=2x_p-1$, so together with the gluon collinear term it reconstructs the standard $\overline{\text{MS}}$ spin-dependent gluon coefficient,
\begin{equation}
...
polarized difference PDF is taken from \texttt{BDSSV24-NNLO}.
...
directly from the extracted run summaries, with all entries expressed in pb and
one-standard-deviation uncertainties quoted for the \Herwig\ and \POLDIS\
columns.
...
\begin{table}[t]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    \hline
    $\gamma$   & $\sigma_{0,\gamma}$              & $2952.10 \pm 0.10$        & $2952.50 \pm 0.20$        & $0.99986$ \\
    $\gamma$   & $\Delta\sigma_{\gamma}$          & $45.500 \pm 0.071$        & $45.4269 \pm 0.0028$      & $1.0016$ \\
    $Z$        & $\sigma_{0,Z}$                   & $1.211040 \pm 0.000037$   & $1.210347 \pm 0.000119$   & $1.00057$ \\
    $Z$        & $\Delta\sigma_{LL,Z}$            & $0.054710 \pm 0.000037$   & $0.054645 \pm 0.000010$   & $1.00119$ \\
    $\gamma Z$ & $\sigma_{0,\gamma Z}$            & $2965.43 \pm 0.07$        & $2965.62 \pm 0.20$        & $0.99993$ \\
    $\gamma Z$ & $\Delta\sigma_{LL,\gamma Z}$     & $50.025 \pm 0.066$        & $50.1405 \pm 0.0031$      & $0.9977$ \\
    int.       & $\sigma_{0,\mathrm{int}}$        & $12.11 \pm 0.12$          & $11.91 \pm 0.28$          & $1.017$ \\
    int.       & $\Delta\sigma_{LL,\mathrm{int}}$ & $4.470 \pm 0.097$         & $4.659 \pm 0.004$         & $0.960$ \\
    \hline
  \end{tabular*}
  \caption{Integrated neutral-current LO cross sections and ratios.  The interference entries
...
\begin{table}[t]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    Setup & Observable & \Herwig\ [pb] & \POLDIS\ [pb] & HW/POLDIS \\
    \hline
    $\gamma$   & $\sigma_{0,\gamma}$              & $2756.77 \pm 0.10$        & $2756.01 \pm 0.23$        & $1.00028$ \\
    $\gamma$   & $\Delta\sigma_{\gamma}$          & $43.468 \pm 0.071$        & $43.6695 \pm 0.0030$      & $0.9954$ \\
    $Z$        & $\sigma_{0,Z}$                   & $1.154301 \pm 0.000032$   & $1.153437 \pm 0.000118$   & $1.00075$ \\
    $Z$        & $\Delta\sigma_{LL,Z}$            & $0.050036 \pm 0.000032$   & $0.050139 \pm 0.000010$   & $0.99794$ \\
    $\gamma Z$ & $\sigma_{0,\gamma Z}$            & $2769.13 \pm 0.05$        & $2768.30 \pm 0.23$        & $1.00030$ \\
    $\gamma Z$ & $\Delta\sigma_{LL,\gamma Z}$     & $47.918 \pm 0.050$        & $48.2886 \pm 0.0032$      & $0.9923$ \\
    int.       & $\sigma_{0,\mathrm{int}}$        & $11.20 \pm 0.11$          & $11.14 \pm 0.32$          & $1.006$ \\
    int.       & $\Delta\sigma_{LL,\mathrm{int}}$ & $4.400 \pm 0.087$         & $4.569 \pm 0.004$         & $0.963$ \\
    \hline
  \end{tabular*}
  \caption{Integrated neutral-current NLO cross sections and ratios.  The NLO values are formed
...
\begin{table}[t]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    \hline
    LO  & $\sigma_{0,W}$              & $9.43500 \pm 0.00559$    & $9.431494 \pm 0.003275$ & $1.00037$ \\
    LO  & $\Delta\sigma_{LL,W}$       & $3.16500 \pm 0.00559$    & $3.157534 \pm 0.001497$ & $1.00236$ \\
    NLO & $\sigma_{0,W}$              & $9.04302 \pm 0.00530$    & $9.038495 \pm 0.003235$ & $1.00050$ \\
    NLO & $\Delta\sigma_{LL,W}$       & $3.02874 \pm 0.00530$    & $3.035988 \pm 0.001456$ & $0.99761$ \\
    \hline
  \end{tabular*}
...
For the unpolarized $Q^2$, $x_{\mathrm{Bj}}$, and $y$ spectra (Fig.~\ref{fig:validation-diff-inclusive-unpol}), the \Herwig\ and \POLDIS\ predictions agree at the few-percent level across the full kinematic range, with the NLO scale band providing a good envelope.
The corresponding polarized distributions (Fig.~\ref{fig:validation-diff-inclusive-pol}) show a comparable level of agreement, confirming that the spin-dependent contributions are correctly implemented.
...
$\sigma(O)$ the corresponding unpolarized combination in the same observable
