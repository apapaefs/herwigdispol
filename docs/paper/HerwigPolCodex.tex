...
In this work we primarily generate the \emph{physical cross section for polarized beams} rather than the spin-difference observable $\Delta\sigma$.\footnote{For selected validation samples the four longitudinal helicity combinations and the derived $\sigma$ and $\Delta\sigma$ combinations are also made available by event reweighting. These named weights can be read either from the HepMC event record or directly by the custom Rivet analyses described in Appendix~\ref{app:rivet-workflow}.}
...
This $Q^2$ window matches the active DIS validation cards in the repository, where the neutral-current and charged-current cuts use \texttt{MinQ2}$=100~\mathrm{GeV}^2$ and \texttt{MaxQ2}$=2500~\mathrm{GeV}^2$.
...
close as practical to the \POLDIS\ fixed-order reference. For the main
...
of Ref.~\cite{Ball:2022proton}, together with the polarized-difference set
\texttt{NNPDFpol20\_nlo\_as\_01180}, based on the NNPDF~pol~2.0 determination
of Ref.~\cite{Cruz-Martinez:2025nnpdfpol20}. This provides a natural
NNPDF-family input combination for the LO and NLO \Herwig-vs-\POLDIS\
implementation checks while keeping the validation setup close to the active
\Herwig\ workflow defaults. As in the rest of the validation, we intentionally
use the same PDF inputs at LO and NLO, since the aim is to isolate
implementation differences rather than perform an order-matched PDF
phenomenology study.
This setup also provides a useful stress test of the polarized projector
construction, since the chosen polarized PDFs can change sign across the
relevant $x$ range. In the implementation the polarized terms enter through the
...
For comparison, we also repeated the broad-window study with the historical
\POLDIS-oriented hybrid choice
\texttt{PDF4LHC15\_nnlo\_100\_pdfas}~\cite{Butterworth:2015oua} and
\texttt{BDSSV24-NNLO}~\cite{Borsa:2024mss}, which were the defaults used in our
earlier \POLDIS\ reference workflow. The resulting integrated cross sections
show similarly good agreement, so we interpret the remaining tiny offsets as
compatible with residual input-level convention differences rather than with a
substantive physics discrepancy. The polarized and unpolarized PDFs are still
not obtained from a single simultaneous fit, so the ratio $\Delta f/f$ should
not be interpreted as a strictly bounded event-level polarization variable
throughout the full sampled phase space.
...
$\alpha_s$ choice specified in the generator setup. This effect is numerically
...
The \texttt{MC\_DIS\_BREIT} analysis and the corresponding campaign-level
combination of the correlated helicity weights are described in
Appendix~\ref{app:rivet-workflow}.
...
\begin{table}[t]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    \hline
    $\gamma$   & $\sigma_{0,\gamma}$              & $2933.65 \pm 0.04$        & $2933.78 \pm 0.14$        & $0.99996 \pm 0.00005$ \\
    $\gamma$   & $\Delta\sigma_{\gamma}$          & $51.044 \pm 0.025$        & $51.0417 \pm 0.0021$      & $1.00004 \pm 0.00049$ \\
    $Z$        & $\sigma_{0,Z}$                   & $1.232712 \pm 0.000008$   & $1.232776 \pm 0.000086$   & $0.99995 \pm 0.00007$ \\
    $Z$        & $\Delta\sigma_{LL,Z}$            & $0.048296 \pm 0.000008$   & $0.048301 \pm 0.000006$   & $0.99989 \pm 0.00021$ \\
    $\gamma Z$ & $\sigma_{0,\gamma Z}$            & $2946.34 \pm 0.02$        & $2946.46 \pm 0.14$        & $0.99996 \pm 0.00005$ \\
    $\gamma Z$ & $\Delta\sigma_{LL,\gamma Z}$     & $56.041 \pm 0.018$        & $56.0312 \pm 0.0023$      & $1.00017 \pm 0.00032$ \\
    int.       & $\sigma_{0,\mathrm{int}}$        & $11.46 \pm 0.04$          & $11.45 \pm 0.20$          & $1.0007 \pm 0.0177$ \\
    int.       & $\Delta\sigma_{LL,\mathrm{int}}$ & $4.949 \pm 0.031$         & $4.9412 \pm 0.0031$       & $1.0016 \pm 0.0062$ \\
    \hline
  \end{tabular*}
...
\begin{table}[t]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    Setup & Observable & \Herwig\ [pb] & \POLDIS\ [pb] & HW/POLDIS \\
    \hline
    $\gamma$   & $\sigma_{0,\gamma}$              & $2732.04 \pm 0.03$        & $2731.90 \pm 0.16$        & $1.00006 \pm 0.00006$ \\
    $\gamma$   & $\Delta\sigma_{\gamma}$          & $48.642 \pm 0.024$        & $48.6255 \pm 0.0024$      & $1.00033 \pm 0.00049$ \\
    $Z$        & $\sigma_{0,Z}$                   & $1.175951 \pm 0.000007$   & $1.175938 \pm 0.000085$   & $1.00001 \pm 0.00007$ \\
    $Z$        & $\Delta\sigma_{LL,Z}$            & $0.044243 \pm 0.000007$   & $0.044247 \pm 0.000006$   & $0.99991 \pm 0.00021$ \\
    $\gamma Z$ & $\sigma_{0,\gamma Z}$            & $2743.91 \pm 0.02$        & $2743.77 \pm 0.16$        & $1.00005 \pm 0.00006$ \\
    $\gamma Z$ & $\Delta\sigma_{LL,\gamma Z}$     & $53.498 \pm 0.017$        & $53.5094 \pm 0.0025$      & $0.99978 \pm 0.00032$ \\
    int.       & $\sigma_{0,\mathrm{int}}$        & $10.69 \pm 0.04$          & $10.70 \pm 0.23$          & $0.9996 \pm 0.0215$ \\
    int.       & $\Delta\sigma_{LL,\mathrm{int}}$ & $4.812 \pm 0.029$         & $4.8396 \pm 0.0034$       & $0.9942 \pm 0.0060$ \\
    \hline
  \end{tabular*}
...
neutral-current channels. For the dominant pure-photon, pure-$Z$, and full
neutral-current cross sections, the ratios HW/POLDIS are consistent with unity
at the permille level or better at both LO and NLO, and the polarized
combinations are likewise statistically compatible in all three setups. The
interference contributions remain the most cancellation-sensitive observables,
especially in the polarized channel, but even there the residual offsets are
well within the combined statistical uncertainties of the present sample. We
therefore interpret the remaining differences as small convention-level input
mismatches, such as PDF or electroweak input choices, rather than as evidence
for a remaining implementation problem.
...
\begin{table}[!hpt]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|ccc|@{}}
    ...
    \hline
    LO  & $\sigma_{0,W}$              & $9.489925 \pm 0.000140$  & $9.48614 \pm 0.00104$   & $1.000399 \pm 0.000111$ \\
    LO  & $\Delta\sigma_{LL,W}$       & $3.002892 \pm 0.000140$  & $3.002497 \pm 0.000430$ & $1.000132 \pm 0.000151$ \\
    NLO & $\sigma_{0,W}$              & $9.0962825 \pm 0.0000978$ & $9.09338 \pm 0.00103$  & $1.000319 \pm 0.000114$ \\
    NLO & $\Delta\sigma_{LL,W}$       & $2.8945279 \pm 0.0000978$ & $2.894812 \pm 0.000419$ & $0.999902 \pm 0.000149$ \\
    \hline
  \end{tabular*}
...
DIS configuration.
The \texttt{MC\_DIS\_PS} analysis used for these shower-level observables, and
its relation to the same correlated-weight machinery used in the fixed-order
validation, is summarized in Appendix~\ref{app:rivet-workflow}.
...
\section{Validation Analyses and Reproducibility Workflow}
\label{app:rivet-workflow}

This appendix summarizes the analysis definitions and workflow used to obtain
the fixed-order and shower-level validation results presented in the main text.
The analysis code is distributed with the companion repository,
\url{https://github.com/apapaefs/herwigdispol}. The two Rivet analyses used here
are \texttt{MC\_DIS\_BREIT} and \texttt{MC\_DIS\_PS}, whose sources are located
under \path{analyses/rivet/dis/}. The reproducibility layer, including the
campaign driver, conversion utilities, HepMC/Rivet-weight handling, and plotting
post-processing scripts, is collected under \path{workflow/dispol/scripts/}.
In particular, \path{run_validation_campaign.py} is the main campaign entry
point, while the additional driver and helper scripts in that directory provide
the conversion from \POLDIS\ output to YODA, the processing of raw POWHEG
summaries, and the construction of validation plots. The compiled Rivet plugins
are generated from the distributed sources using the standard
\texttt{rivet-buildplugin} workflow and are not tracked as binary artifacts.

\subsection{Breit-Frame Fixed-Order Analysis}

The \texttt{MC\_DIS\_BREIT} analysis defines the fixed-order-like comparison to
\POLDIS. It reconstructs the DIS kinematics with the Rivet DIS projection,
boosts the relevant objects to the Breit frame, and clusters the partonic or
hadronic objects specified by the analysis configuration. The \texttt{JETINPUT}
option selects among full final-state jets, hard-parton jets, the leading hard
partons, and the matrix-element-parton mode used in the correlated-weight
validation. For the neutral-current correlated-weight validation the analysis is
run as
\begin{quote}
\small\path{MC_DIS_BREIT:JETINPUT=MEPARTONS:RIVETWEIGHTS=YES}.
\end{quote}

The common DIS acceptance is
$100~\mathrm{GeV}^2 < Q^2 \le 2500~\mathrm{GeV}^2$ and
$0.2 \le y \le 0.6$. After this selection, the analysis records the inclusive
DIS control distributions, including $Q^2$, $x_{\mathrm{Bj}}$, $y$, and the
transverse momenta of the two leading Breit-frame jets. These preselection
histograms are filled before the final dijet requirement; if fewer than two
clustered jets are present, the missing leading-jet transverse momenta are
filled with zero. The selected dijet histograms require two Breit-frame jets
with
$p_{T,\mathrm{jet1}}^B>5~\mathrm{GeV}$ and
$p_{T,\mathrm{jet2}}^B>4~\mathrm{GeV}$, and both selected jets must satisfy
$-3.5<y_{\mathrm{lab}}<3.5$ after boosting back to the laboratory frame. The
resulting validation observables include $Q^2$, $x_{\mathrm{Bj}}$,
$\langle p_T\rangle_2$, $M_{jj}$, $\eta^\ast$, $\zeta_2$,
$p_{T,\mathrm{jet1}}^B$, $p_{T,\mathrm{jet2}}^B$,
$p_{T,\mathrm{jet2}}^B/p_{T,\mathrm{jet1}}^B$, and the jet
transverse-momentum asymmetry.

In correlated-weight mode the analysis reads the named optional weights
\path{HERWIG_DIS_SIGMA} and \path{HERWIG_DIS_DELTA_LL}. The weight lookup first
uses the names supplied through the Rivet event handler and then falls back to
the HepMC3 \texttt{GenEvent} weight table. Thus the same analysis can be applied
either directly during event generation or to an intermediate HepMC file. The
unpolarized histograms are filled with \path{HERWIG_DIS_SIGMA}; the associated
polarized histograms are filled with \path{HERWIG_DIS_DELTA_LL}. The
normalization is then fixed at finalization by the Rivet cross section and the
accumulated default-event weight.

\subsection{Shower-Level Analysis}

\texttt{MC\_DIS\_PS} applies the same DIS acceptance and leading-dijet
selection to observables that are sensitive to subsequent shower evolution. In
the shower-spin validation the correlated-weight cards use
\begin{quote}
\small\path{MC_DIS_PS:JETINPUT=FULL:RIVETWEIGHTS=YES},
\end{quote}
so that the selected jets are clustered from the full final state. After the
leading dijet is selected, the analysis constructs an accepted-jet collection
with $p_T^B>1~\mathrm{GeV}$ and the same laboratory rapidity acceptance. It then
measures the additional radiation through $N_{\mathrm{jets}}$, $p_{T,3}^B$,
$p_{T,3}^B/p_{T,1}^B$, and the scalar sum of transverse momenta carried by jets
beyond the two leading jets.

The particle-level observables are also evaluated in the Breit frame. The
analysis removes the scattered lepton from the visible final state and uses the
current hemisphere to fill the azimuthal distribution relative to the incoming
lepton direction, the corresponding $\cos 2\phi$ moment binned in $Q^2$, and
the current-hemisphere broadening
$B_C=\sum_i p_{T,i}^B/(2\sum_i |\vec{p}_i^B|)$. The third-jet azimuthal
observable is filled only when at least three accepted jets are present. In
\texttt{RIVETWEIGHTS} mode the unpolarized and polarized versions of these
observables use the same \path{HERWIG_DIS_SIGMA} and
\path{HERWIG_DIS_DELTA_LL} weights as \texttt{MC\_DIS\_BREIT}. This construction
correlates the helicity weights with the same showered event, giving a low-noise
validation of the hard-event spin information. It should therefore be viewed as
a validation device rather than as a substitute for independent samples in which
the shower is resampled for each helicity-conditioned hard event.

\subsection{Correlated-Weight Combination}

The campaign driver constructs the correlated-weight validation cards in two
steps. First, it enables \path{GenerateRivetWeights} on the polarized
neutral-current POWHEG matrix element. It then inserts the analysis string
appropriate to the fixed-order or shower-level validation,
\begin{quote}
\small
\path{MC_DIS_BREIT:JETINPUT=MEPARTONS:RIVETWEIGHTS=YES},\\
\path{MC_DIS_PS:JETINPUT=FULL:RIVETWEIGHTS=YES}.
\end{quote}
For the fixed-order-like correlated validation this produces the \texttt{00}
POSNLO and NEGNLO samples, while the four physical helicity combinations and
their derived $\sigma$ and $\Delta\sigma$ combinations are carried as optional
event weights.

The POSNLO--NEGNLO combination is consequently different from the
independent-helicity workflow. In correlated-weight mode the optional weights
already contain the signed target density, and the final NLO histograms are
obtained by adding the normalized POSNLO and NEGNLO \texttt{Estimate1D} bin
values. The driver therefore combines these normalized bins directly, before the
plotted asymmetries are formed. For the shower-spin comparisons the same
machinery is used for the \texttt{RIVETPSWEIGHTS} sample families corresponding
to the \textbf{Full}, \textbf{Born-Only}, and \textbf{None} configurations.
