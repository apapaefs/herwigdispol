...
showered prediction relative to a spin-disabled simulation. Quantitatively, in
the hard-third-jet region ($p_{T,B}^{\mathrm{jet3}}/p_{T,B}^{\mathrm{jet1}}\gtrsim0.7$)
the shower spin correlations reduce $A_{LL}(r_3)$ by up to $\sim$4\% relative to
\textbf{None}, both before and after hadronization. Within the
precision of the runs shown here, however, \textbf{Full} and \textbf{Born-Only}
agree to within $\sim$1\% and remain statistically indistinguishable, so the
additional spin information carried by the accepted real-emission state is not
resolved for the DIS setup and observables considered here. The same conclusion holds for the additional
fourth-jet angular and asymmetry observables that we examined (not shown here).
...
process, which is common to all three configurations, these effects first
become visible in the radiation that resolves a third jet, consistent
...
construction. The two settings share identical hardest-emission kinematics,
azimuthal orientation, flavour channels, and event weights, so any
difference must arise from correlations between the radiation that resolves
a third jet and the hardest-emission system, and these are suppressed for
several independent reasons. The small-$r_3$ region that dominates the
...
accepted real-emission configuration is not resolved as a separate effect within
the present observable set.
...
emission is identical in the two settings and the residual correlations
are soft-suppressed, kernel-suppressed for longitudinal polarization, and
further diluted by showering, clustering, and hadronization.
