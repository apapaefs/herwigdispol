...
\textsc{FastJet}~\cite{Cacciari:2011ma} (3.5.1). The resulting jets are ordered in
...
We also checked additional fourth-jet angular and asymmetry observables, which
...
rather than its azimuthal orientation, at first order in the beam
polarizations: the weight survives azimuthal integration, its
...
difference must arise from correlations between the second resolved
emission and the hardest-emission system, and these are suppressed for
...
polarization of the emitted gluon, whose azimuthal imprint on a subsequent
...
channels and, within the quoted uncertainties, at sub-percent to percent-level
precision for the smaller polarized interference contributions.
...
the Born-level spin-density initialization can have a visible impact on
shower-sensitive observables, while the additional spin information from the
accepted real-emission configuration is not resolved as a separate effect within
the present observable set. We also checked fourth-jet angular and asymmetry
observables, omitted here for clarity, and found the same conclusion.
...
and hadronization settings would sharpen this comparison. More
...
here will be made publicly available in a future \Herwig\ release.
