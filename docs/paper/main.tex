...
We perform the fixed-order comparison using the Rivet analysis framework~\cite{Bierlich:2024vqo}, with the \Herwig/Rivet event selection matched to the \POLDIS\ setup. We use $E_e=18~\mathrm{GeV}$ and $E_p=275~\mathrm{GeV}$, with the DIS window
...
\textsc{FastJet}~\cite{Cacciari:2011ma}. The resulting jets are ordered in
...
are imposed.
The analysis and PDF interface use \textsc{Rivet}~4.1.1,
\textsc{FastJet}~3.5.1, and \textsc{LHAPDF}~6.5.5~\cite{Buckley:2014ana}.
...
splittings. For longitudinal beam polarization the spin-dependent
azimuthal weights vanish identically ($H_{i+-}=H_{i-+}=0$), so the
polarized observables are sensitive to the new information only through
higher links of the spin-correlation chain. Finally, only third jets
