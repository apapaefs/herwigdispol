...
showered prediction relative to a spin-disabled simulation. Within the
precision of the runs shown here, however, \textbf{Full} and \textbf{Born-Only}
are not distinguishable from each other, so the additional spin information
carried by the accepted real-emission state is not resolved for the DIS setup
and observables considered here. The same conclusion holds for the additional
