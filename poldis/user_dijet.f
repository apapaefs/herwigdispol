...
c      SCALEJET=(zkt(n1)+zkt(n2))**2
      SCALEJET=(Q2+((zkt(n1)+zkt(n2))/2)**2)/2 
      SCALEREN=SCALEJET
      SCALEFACT=SCALEJET 
...
C     P_T,1 DISTRIBUTION
       CALL GBOOK1(1,"pt1 NNLO",15,5d0,40D0,1,0,0,0)
       CALL GBOOK1(2,"pt1 NLO ",15,5d0,40D0,0,0,0,0)
       CALL GBOOK1(3,"pt1 LO  ",15,5d0,40D0,0,0,0,0)
C     ETA_1 DISTRIBUTION      
       CALL GBOOK1(4,"eta1L NNLO",15,-3.5d0,3.5D0,1,0,0,0)
...
C     X DEPENDENCE
       ...
       CALL GBOOK1(12,"X LO  ",15,0.01d0,1.0d0,0,0,1,0)
C     INVARIANT DI-JET MASS DISTRIBUTION 
       CALL GBOOK1(13,"MM NNLO",15,8d0,100d0,1,0,1,0)
       CALL GBOOK1(14,"MM NLO ",15,8d0,100d0,0,0,1,0)
       CALL GBOOK1(15,"MM LO  ",15,8d0,100d0,0,0,1,0)
C     AVERAGE PT DISTRIBUTION
       CALL GBOOK1(16,"<pt> NNLO",15,4.5d0,30d0,1,0,0,0)
       CALL GBOOK1(17,"<pt> NLO ",15,4.5d0,30d0,0,0,0,0)
       CALL GBOOK1(18,"<pt> LO  ",15,4.5d0,30d0,0,0,0,0)
C     ETA* DISTRIBUTION
       CALL GBOOK1(19,"eta* NNLO",15,0d0,2.5d0,1,0,0,0)
       CALL GBOOK1(20,"eta* NLO ",15,0d0,2.5d0,0,0,0,0)
       CALL GBOOK1(21,"eta* LO  ",15,0d0,2.5d0,0,0,0,0)
C     LOG(XI_2) DISTRIBUTION
       CALL GBOOK1(22,"log(e2) NNLO",15,-2.1d0,0d0,1,0,0,0)
       CALL GBOOK1(23,"log(e2) NLO ",15,-2.1d0,0d0,0,0,0,0)
       CALL GBOOK1(24,"log(e2) LO  ",15,-2.1d0,0d0,0,0,0,0) 

C     P_T,2 DISTRIBUTION
       CALL GBOOK1(25,"pt2 NNLO",15,4d0,40D0,1,0,0,0)
       CALL GBOOK1(26,"pt2 NLO ",15,4d0,40D0,0,0,0,0)
       CALL GBOOK1(27,"pt2 LO  ",15,4d0,40D0,0,0,0,0)
C     ETA_2 DISTRIBUTION      
       CALL GBOOK1(28,"eta2L NNLO",15,-3.5d0,3.5D0,1,0,0,0)
       CALL GBOOK1(29,"eta2L NLO ",15,-3.5d0,3.5D0,0,0,0,0)
       CALL GBOOK1(30,"eta2L LO  ",15,-3.5d0,3.5D0,0,0,0,0)
...
C---   Di-jet variables
     ...
     $     .and.yeta(n2).gt.-3.5d0.and.yeta(n2).lt.3.5d0)then
        ...
        CALL GFILL1(1,max(zkt(n1),zkt(n2)),W(2))
        CALL GFILL1(2,max(zkt(n1),zkt(n2)),W(1))
        CALL GFILL1(3,max(zkt(n1),zkt(n2)),W(0))
        ...
        CALL GFILL1(16,(zkt(n1)+zkt(n2))/2,W(2))               
        CALL GFILL1(17,(zkt(n1)+zkt(n2))/2,W(1))
        CALL GFILL1(18,(zkt(n1)+zkt(n2))/2,W(0))
        ...
        CALL GFILL1(25,min(zkt(n1),zkt(n2)),W(2))
        CALL GFILL1(26,min(zkt(n1),zkt(n2)),W(1))
        CALL GFILL1(27,min(zkt(n1),zkt(n2)),W(0))

        CALL GFILL1(28,yeta(n2),W(2))
        CALL GFILL1(29,yeta(n2),W(1))
        CALL GFILL1(30,yeta(n2),W(0))
                   
C     COMPUTE INCLUSIVE DI-JET X-SECTION             
             CS(7)=CS(7)+W(2)
             CS(8)=CS(8)+W(1)
             CS(9)=CS(9)+W(0)                                 
       ENDIF             
