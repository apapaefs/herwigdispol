C#######################################################################
C     POLDIS: USER RUTINE (P2B COMPONENT AUDIT)
C            . DEFINE MAIN SETTING FOR THE CALCULATION
C            . DEFINE CUTS AND HISTOGRAMS
C            . PRINT CALLBACK-LEVEL P2B COMPONENT TOTALS
C#######################################################################
      IMPLICIT NONE
...
C-----------------------------------------------------------------------
      IF (IBOSON .EQ. 2) THEN
        IF (IPOL .EQ. 1) THEN
          TITLE = "dijets_pol_Z_p2bc.top"
        ELSE
          TITLE = "dijets_unpol_Z_p2bc.top"
        ENDIF
      ELSEIF (IBOSON .EQ. 0) THEN
        IF (IPOL .EQ. 1) THEN
          TITLE = "dijets_pol_GAM_p2bc.top"
        ELSE
          TITLE = "dijets_unpol_GAM_p2bc.top"
        ENDIF
      ELSEIF (IBOSON .EQ. 3) THEN
        IF (IPOL .EQ. 1) THEN
          TITLE = "dijets_pol_W_p2bc.top"
        ELSE
          TITLE = "dijets_unpol_W_p2bc.top"
        ENDIF
      ELSE
        IF (IPOL .EQ. 1) THEN
          TITLE = "dijets_pol_p2bc.top"
        ELSE
          TITLE = "dijets_unpol_p2bc.top"
        ENDIF
      ENDIF
...
C     Callback-level component bookkeeping for P2B decomposition.
...
C     Guard against tiny negative values from rounding
      ...
      WRITE(*,'(/A)') 'POLDIS callback component summary'
      WRITE(*,'(A)') '================================'
      DO K=1,NCOMP
        SIGCLO   = COMPSUM(1,K)
        SIGCNLO  = COMPSUM(2,K)
  ...
  200   CONTINUE
      ...
      INTEGER FUNCTION COMPONENT_SLOT(NA,ITYPE,N)
      IMPLICIT NONE
      INTEGER NA,ITYPE,N

      COMPONENT_SLOT=0
      IF (NA.EQ.0 .AND. ITYPE.EQ.0 .AND. N.EQ.5) THEN
        COMPONENT_SLOT=1
      ELSEIF (NA.EQ.1 .AND. ITYPE.EQ.0) THEN
        COMPONENT_SLOT=2
      ELSEIF (NA.EQ.1 .AND. ITYPE.EQ.1) THEN
        COMPONENT_SLOT=3
      ELSEIF (NA.EQ.1 .AND. ITYPE.EQ.2) THEN
        COMPONENT_SLOT=4
      ELSEIF (NA.EQ.1 .AND. ITYPE.EQ.3) THEN
        COMPONENT_SLOT=5
      ELSEIF (NA.EQ.1 .AND. ITYPE.EQ.5) THEN
        COMPONENT_SLOT=6
      ELSEIF (NA.EQ.2 .AND. ITYPE.EQ.0) THEN
        COMPONENT_SLOT=7
      ELSEIF (NA.EQ.2 .AND. ITYPE.EQ.1) THEN
        COMPONENT_SLOT=8
      ELSEIF (NA.EQ.2 .AND. ITYPE.EQ.2) THEN
        COMPONENT_SLOT=9
      ELSEIF (NA.EQ.2 .AND. ITYPE.EQ.3) THEN
        COMPONENT_SLOT=10
      ELSEIF (NA.EQ.2 .AND. ITYPE.EQ.5) THEN
        COMPONENT_SLOT=11
      ENDIF
