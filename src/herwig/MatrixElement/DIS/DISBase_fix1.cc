...
RealEmissionProcessPtr DISBase::generateHardest(RealEmissionProcessPtr born,
  ...
  // coefficient for the matrix elements (use beam polarisations)
  double Pl = 0.0, Pq = 0.0;
...
double DISBase::NLOWeight() const {
  ...
  // Use incoming spin density matrices if available (falls back to 0 if not set)
  double Pl = 0.0, Pq = 0.0;
  {
    // getRhoMatrices() as in MENeutralCurrentDIS usage
    // first  = lepton beam,
    // second = parton from hadron beam
    const std::pair<RhoDMatrix,RhoDMatrix> rho = getRhoMatrices();
    Pl = longPol(rho.first);
    Pq = longPol(rho.second);
  }
  ...
  // Fetch the longitudinal *difference* PDF: Δf(x,Q^2) = f^+(x,Q^2) - f^-(x,Q^2)
  // Provided by ThePEG::PolarizedPartonExtractor via run configuration.
  ThePEG::tcPDFPtr diffPdf;
  {
    auto pe = generator()->eventHandler()->partonExtractor();

    // Cast to PolarizedPartonExtractor
    ThePEG::Ptr<ThePEG::PolarizedPartonExtractor>::pointer ppe =
      ThePEG::dynamic_ptr_cast< ThePEG::Ptr<ThePEG::PolarizedPartonExtractor>::pointer >(pe);
  ...
  if (hasDiffPdf) {
    if (qPDF != 0.0) Pq_m = Pz * (dqPDF / qPDF);
    if (gPDF != 0.0) Pg_m = Pz * (dgPDF / gPDF);
  }
  ...
  // --------------------------------------------------------------------------
  // Born-level polarisation fraction f_pol
  //
  // The Born cross section σ_Born ∝ (1 + a_born·l + l²) where l = 2/y − 1.
  // The polarised part is the a_born·l term, so:
  //   f_pol = a_born·l / (1 + a_born·l + l²)
  //
  // The NLO correction is a multiplicative factor on σ_Born. The collinear
  // counterterms and PDF ratios must use polarised PDFs (Δf) and polarised
  // splitting functions (ΔP) for the fraction f_pol of the cross section.
  // --------------------------------------------------------------------------

  // calculate lepton kinematic variables (needed for f_pol and real emission)
  Lorentz5Momentum q = meMomenta()[0]-meMomenta()[2];
  double  yB = (q*meMomenta()[1])/(meMomenta()[0]*meMomenta()[1]);
  double l = 2./yB-1.;

  // Born analysing power at xB (uses Pq from rho matrix = Pz*Δq(xB)/q(xB))
  double a_born = A_pol(mePartonData()[0], mePartonData()[2],
                        mePartonData()[1], mePartonData()[3],
                        q2_, Pl, Pq);

  double denom_born = 1. + a_born*l + sqr(l);
  double f_pol = (std::abs(denom_born) > 1e-30) ? a_born*l / denom_born : 0.0;

  // --------------------------------------------------------------------------
  // collinear counterterms
  //
  // The unpolarised collinear terms use:
  //   Pgq_unpol(z) = z² + (1−z)²,  finite_g_unpol = 2z(1−z)
  //   Pqq is the same for polarised and unpolarised
  //   PDF ratios: q(xB/xp)/q(xB) for quark, g(xB/xp) for gluon
  //
  // The polarised collinear terms use:
  //   ΔPgq(z) = 2z − 1,  finite_g_pol = 2(1−z)
  //   Same Pqq kernel
  //   PDF ratios: Δq(xB/xp)/Δq(xB) for quark, Δg(xB/xp) for gluon
  //
  // We blend: collX = (1−f_pol)·collX_unpol + f_pol·collX_pol
  // --------------------------------------------------------------------------
  ...
  if (hasDiffPdf && std::abs(Pz*dloPDF) > 1e-30) {
    collg_over_born_pol =
      TRfact/xp_*(Pz*dgPDF/(Pz*dloPDF))*(2.*(1.-xp_) + (2.*xp_-1.)*logRatio);
  }

  double collg_over_born = (1.-f_pol)*collg_over_born_unpol + f_pol*collg_over_born_pol;
  ...
  if (hasDiffPdf && std::abs(Pz*dloPDF) > 1e-30) {
    double dqRatio = Pz*dqPDF / (Pz*dloPDF);
    collq_over_born_pol =
  ...
  double collq_over_born = (1.-f_pol)*collq_over_born_unpol + f_pol*collq_over_born_pol;

  // --------------------------------------------------------------------------
  // real-emission pieces: QCDC (q→qg) and BGF (g→qqbar)
  //
  // The ME kernel already correctly handles polarisation via the analysing
  // power a_q, a_g. But the PDF flux ratios (qPDF/loPDF, gPDF/loPDF) must
  // also be corrected: the polarised fraction uses Δf/Δf ratios.
  //
  // Effective PDF ratios:
  //   qRatio = (1−f_pol)·q(x)/q(xB) + f_pol·Pz·Δq(x)/(Pz·Δq(xB))
  //   gRatio = (1−f_pol)·g(x)/q(xB) + f_pol·Pz·Δg(x)/(Pz·Δq(xB))
  // --------------------------------------------------------------------------

  // --------------------------------------------------------------------------
  // Real-emission analysing powers
  //
  // The realq/realg formulas compute σ_real / σ_Born.  The denominator
  // (1+a·l+l²) must use a_born to cancel the Born exactly.  The kernel
  // numerator uses the *mapped* analysing power (at xB/xp), because the
  // incoming parton in the real emission carries that momentum fraction.
  //
  // The polarised luminosity enters through the mapped analysing power
  // (which contains Pz·Δf/f), so the PDF flux ratios use the plain
  // unpolarised PDFs — no f_pol blending.  Blending would double-count
  // the polarisation already carried by the analysing power.
  // --------------------------------------------------------------------------
  ...
  // BGF two-analysing-power decomposition.
  // The kernel K(z,l) = Pgq(z)*(1+2al+l^2) + 4z(1-z)*(1-l^2), where
  // Pgq = z^2+(1-z)^2 decomposes into R2 (quark, z^2) and R3 (antiquark,
  // (1-z)^2) with DIFFERENT analysing powers:
  //   R2 (quark along beam):    Born is γ*q → q,    a_g_R2 = A(Pl, +Pg)
  //   R3 (antiquark along beam): Born is γ*qbar → qbar, a_g_R3 = A_qbar(Pl, -Pg)
  // Convention: implement the R2/R3 opposite helicity assignment explicitly
  // via Pg -> -Pg in R3, while keeping CC particle data for correct NC couplings.
  // The smooth part 4z(1-z)*(1-l^2) is the FL cross-term between R2 and
  // R3 and is a-independent (the opposite polarisations cancel).
  ...
  // R3: charge-conjugate of the Born parton with explicit opposite gluon pol -Pg
  ...
  double realq = CFfact/xp_/(1.+a_born*l+sqr(l))*qRatio*
    (2.+2.*sqr(l)-xp_+3.*xp_*sqr(l)+a_q_mapped*l*(2.*xp_+1.));

  // g -> q qbar term (BGF): denominator a_born, two analysing powers
  // K = (1+l^2+2(1-3l^2)*z*(1-z)) + 2l*(z^2*a_R2 + (1-z)^2*a_R3)
  double realg = -TRfact/xp_/(1.+a_born*l+sqr(l))*gRatio*
