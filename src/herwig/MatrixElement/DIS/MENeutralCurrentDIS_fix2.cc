...
double MENeutralCurrentDIS::A_pol(tcPDPtr lin, tcPDPtr,
  ...
  // Photon-only: pure γ exchange gives a *double-spin* analysing power.
  // Convention: no extra particle/antiparticle sign flip here.
  // The parton helicity sign is entirely encoded in Pq = Pz * Δf/f.
  if (_gammaZ == 1) {
    return 2.0 * Pl * Pq;
  }

  // Existing NC formula (γ+Z or Z-only)
  const double r = (_gammaZ == 0 || _gammaZ == 2) ? double(q2 / (q2 + _mz2)) : 0.0;
  ...
  if (std::abs(lin->id()) % 2 == 0) { // neutrino line
    Ql  = (_gammaZ == 0) ? etaL * generator()->standardModel()->enu() : 0.0;
    CVl = etaL * 0.25 * generator()->standardModel()->vnu();
    CAl = etaL * 0.25 * generator()->standardModel()->anu();
  } else { // charged lepton line
    Ql  = (_gammaZ == 0) ? etaL * generator()->standardModel()->ee()  : 0.0;
    CVl = etaL * 0.25 * generator()->standardModel()->ve();
    CAl = etaL * 0.25 * generator()->standardModel()->ae();
  }

  if (std::abs(qin->id()) % 2 == 0) { // up-type quark
    Qq  = (_gammaZ == 0) ? etaQ * generator()->standardModel()->eu() : 0.0;
    CVq = etaQ * 0.25 * generator()->standardModel()->vu();
    CAq = etaQ * 0.25 * generator()->standardModel()->au();
  } else { // down-type quark
    Qq  = (_gammaZ == 0) ? etaQ * generator()->standardModel()->ed() : 0.0;
    CVq = etaQ * 0.25 * generator()->standardModel()->vd();
    CAq = etaQ * 0.25 * generator()->standardModel()->ad();
  }
  ...
  // N and D coefficients for the polarized NC analysing power (eq. 25--37 of draft).
  // Code uses tilde-couplings: CVl = v_l/4, CAl = a_l/4, with k = 1/(sW^2 cW^2).
  // The relationship to the draft's C_{V,A} = (T3/2 - Q sW^2)/(sW cW) is
  // C_draft = sqrt(k) * C_tilde, so C^2 = k * C_tilde^2.
  // N0, D0, Nlq are independent of Pl,Pq and were validated with the original A().
  // Nl, Nq, Dl, Dq, Dlq each carry an overall factor of r*k from the Z propagator
  // times coupling normalization, arising from the single/double spin-flip interference.
  const double rk = r * k;

  const double N0  =  4.0 * rk * CAlCAq * ( Ql * Qq + 2.0 * rk * CVlCVq );
  const double Nl  = -4.0 * rk * CAq * ( CVq * rk * (CAl2 + CVl2) + CVl * Ql * Qq );
  const double Nq  = -4.0 * rk * CAl * ( CVl * rk * (CAq2 + CVq2) + CVq * Ql * Qq );
  const double Nlq =  2.0 * ( rk*rk * (CAq2 + CVq2) * (CAl2 + CVl2)
                          +  2.0 * rk * CVlCVq * Ql * Qq
                          +  Ql2 * Qq2 );
  const double D0  =  Ql2 * Qq2
                   +  2.0 * Ql * Qq * rk * CVlCVq
                   +  rk*rk * (CVl2 + CAl2) * (CVq2 + CAq2);
  const double Dl  = -2.0 * rk * CAl * ( rk * CVl * (CAq2 + CVq2) + CVq * Ql * Qq );
  const double Dq  = -2.0 * rk * CAq * ( rk * CVq * (CAl2 + CVl2) + CVl * Ql * Qq );
  const double Dlq =  2.0 * rk * CAlCAq * ( 2.0 * rk * CVlCVq + Ql * Qq );

  double num = N0 + Pl * Nl + Pq * Nq + Pl * Pq * Nlq;
  double den = D0 + Pl * Dl + Pq * Dq + Pl * Pq * Dlq;
