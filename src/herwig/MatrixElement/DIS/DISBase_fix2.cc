...
double DISBase::NLOWeight() const {
  ...
  // Smooth division helper (no hard zeroing in polarized ratios).
  const double tiny = 1e-30;
  auto safeDiv = [tiny](double num, double den) {
    if (std::abs(den) > tiny) return num / den;
    return num / (den >= 0.0 ? tiny : -tiny);
  };
  ...
  if (hasDiffPdf) {
    if (std::abs(qPDF) > 1e-12) Pq_m = Pz * (dqPDF / qPDF);
    else Pq_m = 0.0;

    if (std::abs(gPDF) > 1e-12) Pg_m = Pz * (dgPDF / gPDF);
    else Pg_m = 0.0;
  }
  ...
  const double denom_born = 1. + a_born*l + l2;
  const double bornU = 1. + l2;
  const double bornP = a_born*l;
  const double invBorn = safeDiv(1.0, denom_born);

  double logRatio = log((1.-xp_)*q2_/xp_/mu2);
  const double qRatio = safeDiv(qPDF, loPDF);
  const double gRatio = safeDiv(gPDF, loPDF);
  double dqRatio = 0.0;
  double dgRatio = 0.0;
  // Protect Delta-ratios against nodes of Delta q(xB):
  // bornP * (Delta f(x)/Delta q(xB)) is finite, but the raw ratio can spike
  // numerically when Delta q(xB) is tiny.
  const double minDlo = std::max(1e-12, 1e-4 * std::abs(loPDF));
  if (hasDiffPdf && std::abs(dloPDF) > minDlo) {
    dqRatio = dqPDF / dloPDF;
    dgRatio = dgPDF / dloPDF;
  }

  // Collinear U/P pieces assembled at numerator level, then projected once.
  const double collgU =
    TRfact/xp_*gRatio*(2.*xp_*(1.-xp_) + (sqr(xp_)+sqr(1.-xp_))*logRatio);
  const double collgP =
    TRfact/xp_*dgRatio*(2.*(1.-xp_) + (2.*xp_-1.)*logRatio);
  const double collqU =
    CFfact/xp_*qRatio*(1.-xp_-2./(1.-xp_)*log(xp_)-(1.+xp_)*log((1.-xp_)/xp_*q2_/mu2))+
    CFfact/xp_*(qRatio-xp_)*(2./(1.-xp_)*log(q2_*(1.-xp_)/mu2)-1.5/(1.-xp_));
  const double collqP =
  ...
  const double coll = invBorn * (bornU*(collqU + collgU) + bornP*(collqP + collgP));
  ...
  // Real U/P pieces assembled first, then projected once.
  const double realqU_num = CFfact/xp_ * qRatio *
    (2. + 2.*l2 - xp_ + 3.*xp_*l2);
  const double realqP_num = CFfact/xp_ * qRatio *
    (a_q_mapped*l*(2.*xp_+1.));
  const double realgU_num = -TRfact/xp_ * gRatio *
    (1. + l2 + 2.*(1.-3.*l2)*xp_*(1.-xp_));
  const double realgP_num = -TRfact/xp_ * gRatio *
    (2.*l*(sqr(xp_)*a_g_R2+sqr(1.-xp_)*a_g_R3));
  const double real = invBorn * (realqU_num + realqP_num + realgU_num + realgP_num);
