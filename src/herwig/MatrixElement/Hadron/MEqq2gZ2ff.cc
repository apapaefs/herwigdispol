...
double MEqq2gZ2ff::me2() const {
  ...
  return qqbarME(rhoin,fin,ain,fout,aout,false);
}
...
double MEqq2gZ2ff::qqbarME(const pair<RhoDMatrix,RhoDMatrix> & rhoin ,
  ...
  ProductionMatrixElement Zboson(PDT::Spin1Half,PDT::Spin1Half,
				 PDT::Spin1Half,PDT::Spin1Half);
  // which intermediates to include
  bool haveGamma=_gammaZ==0||_gammaZ==1;
  bool haveZ0   =_gammaZ==0||_gammaZ==2;
  ...
  for(ihel1=0;ihel1<2;++ihel1) {
    for(ihel2=0;ihel2<2;++ihel2) {
      // intermediate for Z
      if(haveZ0)    inter[0]=_theFFZVertex->evaluate(mb2,1,_z0   ,fin[ihel1],ain[ihel2]);
      // intermediate for photon
      if(haveGamma) inter[1]=_theFFPVertex->evaluate(mb2,1,_gamma,fin[ihel1],ain[ihel2]);
	...
	for(ohel2=0;ohel2<2;++ohel2) {
	  ...
	  Zboson(ihel1,ihel2,ohel1,ohel2) = diag1;
	  // first the photon exchange diagram
	  diag2 = haveGamma ?
	    _theFFPVertex->evaluate(mb2,aout[ohel2],fout[ohel1],inter[1]) : 0.;
	  gamma (ihel1,ihel2,ohel1,ohel2) = diag2;
	  // add up squares of individual terms
	  menew(ihel1,ihel2,ohel1,ohel2) = diag1+diag2;
	}
      }
    }
  }
  // colour factor (spin average now in rho matrix contraction)
  double colspin=1./3.;
  if(abs(fout[0].id())<=6) colspin*=3.;
  double me[3]={colspin*menew .average(rhoin.first,rhoin.second),
                colspin*Zboson.average(rhoin.first,rhoin.second),
                colspin*gamma .average(rhoin.first,rhoin.second)};
...
void MEqq2gZ2ff::constructVertex(tSubProPtr sub) {
  ...
  pair<RhoDMatrix,RhoDMatrix> rhoin = getRhoMatrices();
  qqbarME(rhoin,fin,ain,fout,aout,true);
