...
double MEqq2W2ff::qqbarME(const pair<RhoDMatrix,RhoDMatrix> & rhoin ,
  ...
  for(ihel1=0;ihel1<2;++ihel1) {
	...
	for(ohel2=0;ohel2<2;++ohel2) {
	  if (positive) {
	    ...
	    newme(ihel1,ihel2,ohel1,ohel2) = _theFFWVertex->evaluate(scale(),aout[ohel2],fout[ohel1],inter);
	  } 
	  else {
	    // the Wm exchange
  ...
  return newme.average(rhoin.first,rhoin.second)*colspin;
}
