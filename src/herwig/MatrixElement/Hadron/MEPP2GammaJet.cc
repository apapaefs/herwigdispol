...
double MEPP2GammaJet::me2() const {
  ...
  double me(0.);
  pair<RhoDMatrix,RhoDMatrix> rhoin = getRhoMatrices();
  // first case, q qbar to gluon photon
  if(mePartonData()[0]->id()==-mePartonData()[1]->id()) {
    // check order of the particles
    assert(mePartonData()[0]->id()>0);
    assert(mePartonData()[3]->id()==ParticleID::g);
    ...
    VectorWaveFunction   phout(meMomenta()[2],mePartonData()[2],outgoing);
    VectorWaveFunction   glout(meMomenta()[3],mePartonData()[3],outgoing);
    ...
    me = qqbarME(rhoin,fin,ain,gout,pout,false)*4./9.;
//       Energy2 mt(scale());
...
//  	   << endl;
    ...
    // check order of the particles
    assert(mePartonData()[1]->id()==ParticleID::g    );
    assert(mePartonData()[2]->id()==ParticleID::gamma);
    // calculate the spinors and polarization vectors
    vector<SpinorWaveFunction> fin;
    ...
    // calculate the matrix element
    me = qgME(rhoin,fin,gin,pout,fout,false)/6.;
//       Energy2 mt(scale());
//     double coupling=sqr(4.*Constants::pi)*SM().alphaEM(ZERO)*SM().alphaS(mt);
...
// 	   << endl; 
    ...
    vector<VectorWaveFunction> pout,gin;
    SpinorBarWaveFunction  qin (meMomenta()[0],mePartonData()[0],incoming);
    ...
    // calculate the matrix element
    me=qbargME(rhoin,ain,gin,pout,aout,false)/6.;
//       Energy2 mt(scale());
//     double coupling=sqr(4.*Constants::pi)*SM().alphaEM(ZERO)*SM().alphaS(mt);
//       Energy2 s(sHat()),t(tHat()),u(uHat());
//       double me2=-1./3./s/t*(s*s+t*t+2.*u*(s+t+u))*coupling*
...
double MEPP2GammaJet::qqbarME(const pair<RhoDMatrix,RhoDMatrix> & rhoin ,
                              const vector<SpinorWaveFunction>    & fin,
			      const vector<SpinorBarWaveFunction> & ain,
			      const vector<VectorWaveFunction>    & gout,
			      const vector<VectorWaveFunction>    & pout,
  ...
  ProductionMatrixElement newme(PDT::Spin1Half,PDT::Spin1Half,
				PDT::Spin1,PDT::Spin1);
  ProductionMatrixElement me1(PDT::Spin1Half,PDT::Spin1Half,
                              PDT::Spin1,PDT::Spin1);
  ProductionMatrixElement me2(PDT::Spin1Half,PDT::Spin1Half,
                              PDT::Spin1,PDT::Spin1);
  ...
  for(inhel1=0;inhel1<2;++inhel1) {
	...
	for(outhel2=0;outhel2<2;++outhel2) {
	  ...
	  me1(inhel1,inhel2,2*outhel1,2*outhel2) = diag1 = _photonvertex->evaluate(ZERO,inter,ain[inhel2],pout[outhel2]);
	  // second diagram
	  inter = _photonvertex->evaluate(ZERO,5,fin[inhel1].particle()->CC(),
					  fin[inhel1],pout[outhel2]);
	  me2(inhel1,inhel2,2*outhel1,2*outhel2) = diag2 = _gluonvertex->evaluate(mt,inter,ain[inhel2],gout[outhel1]);
	  // sum
	  newme(inhel1,inhel2,2*outhel1,2*outhel2)= diag1+diag2;
	}
      }		
    }
  }
  ...
  if(!calc) {
    DVector save;
    save.push_back(me1.average(rhoin.first,rhoin.second));
    save.push_back(me2.average(rhoin.first,rhoin.second));
    meInfo(save);
  }
  // return the answer
  if(calc) _me.reset(newme);
  return newme.average(rhoin.first,rhoin.second);
}

double MEPP2GammaJet::qgME(const pair<RhoDMatrix,RhoDMatrix> & rhoin ,
                           const vector<SpinorWaveFunction>    & fin,
			   const vector<VectorWaveFunction>    & gin,
			   const vector<VectorWaveFunction>    & pout,
			   const vector<SpinorBarWaveFunction> & fout,
			   const bool calc) const {
  // the particles should be in the order
  // for the incoming 
  // 0 incoming fermion     (u    spinor)       
  ...
  ProductionMatrixElement me1(PDT::Spin1Half,PDT::Spin1,
                              PDT::Spin1,PDT::Spin1Half);
  ProductionMatrixElement me2(PDT::Spin1Half,PDT::Spin1,
                              PDT::Spin1,PDT::Spin1Half);
  // wavefunction for the intermediate particles
  SpinorWaveFunction inter;
  unsigned int inhel1,inhel2,outhel1,outhel2;
  ...
  for(inhel1=0;inhel1<2;++inhel1) {
	...
	for(outhel2=0;outhel2<2;++outhel2) {
	  ...
	  me1(inhel1,2*inhel2,2*outhel1,outhel2) = diag1 = _gluonvertex->evaluate(mt,inter,fout[outhel2],gin[inhel2]);
	  // second diagram
	  inter = _gluonvertex->evaluate(mt,5,fin[inhel1].particle()->CC(),
					 fin[inhel1],gin[inhel2]);
	  me2(inhel1,2*inhel2,2*outhel1,outhel2) = diag2 = _photonvertex->evaluate(ZERO,inter,fout[outhel2],pout[outhel1]);
	  // matrix element
	  newme(inhel1,2*inhel2,2*outhel1,outhel2) = diag1 + diag2;
	}
      }
    }
  }
...
double MEPP2GammaJet::qbargME(const pair<RhoDMatrix,RhoDMatrix> & rhoin ,
                              const vector<SpinorBarWaveFunction> & ain,
			      const vector<VectorWaveFunction>    & gin,
			      const vector<VectorWaveFunction>    & pout,
			      const vector<SpinorWaveFunction>    & aout,
			      const bool calc) const {
  // the particles should be in the order
  // for the incoming 
  ...
  for(inhel1=0;inhel1<2;++inhel1) {
	...
	for(outhel2=0;outhel2<2;++outhel2) {
	  ...
	  me1(inhel1,2*inhel2,2*outhel1,outhel2) = diag1 = _gluonvertex->evaluate(mt,aout[outhel2],inter,gin[inhel2]);
	  // second diagram
	  inter = _gluonvertex->evaluate(mt,5,ain[inhel1].particle()->CC(),
					 ain[inhel1],gin[inhel2]);
	  me2(inhel1,2*inhel2,2*outhel1,outhel2) = diag2 = _photonvertex->evaluate(ZERO,aout[outhel2],inter,pout[outhel1]);
	  // matrix element
	  newme(inhel1,2*inhel2,2*outhel1,outhel2)= diag1+diag2;
	}
      }		
...
void MEPP2GammaJet::constructVertex(tSubProPtr sub) {
  ...
  if(hard[0]->id()==ParticleID::g||hard[1]->id()==ParticleID::g) {
    ...
    p[1]=p[2];g[1]=g[2];
    qqbarME(rhoin,q,qb,g,p,true);
  }
  hard[order[0]]->spinInfo()->rhoMatrix(rhoin.first );
  hard[order[1]]->spinInfo()->rhoMatrix(rhoin.second);
  for(unsigned int ix=2;ix<4;++ix) {
    if(hard[ix]->id()==21 || hard[ix]->id()==22) {
      RhoDMatrix temp(hard[ix]->dataPtr()->iSpin());
      temp(0,0) = temp(2,2) = 0.5;
      temp(1,1)=0.;
      hard[ix]->spinInfo()->DMatrix(temp);
    }
  }
