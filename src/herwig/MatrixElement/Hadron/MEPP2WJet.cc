...
double MEPP2WJet::me2() const {
  ...
  if(mePartonData()[0]->id()<=6&&mePartonData()[0]->id()>0&&
    ...
    output=qgME(rhoin,fin,gin,fout,lm,lp);
  }
  // qbar g to qbar W
    ...
    output=qbargME(rhoin,ain,gin,aout,lm,lp);
  }
  // q qbar to g W
  else {
    vector<SpinorWaveFunction>     fin;
    ...
    output=qqbarME(rhoin,fin,ain,gout,lm,lp);
  }
  return output*sHat();
}

InvEnergy2 MEPP2WJet::qqbarME(pair<RhoDMatrix,RhoDMatrix> & rhoin,
			      vector<SpinorWaveFunction> & fin,
			      vector<SpinorBarWaveFunction> & ain,
			      vector<VectorWaveFunction> & gout,
			      vector<SpinorBarWaveFunction> & lm,
  ...
  ProductionMatrixElement newme(PDT::Spin1Half,PDT::Spin1Half,
				PDT::Spin1,PDT::Spin1Half,PDT::Spin1Half);
	...
	interb=_theQQGVertex->evaluate(_scale,5,mePartonData()[1],
 	...
 	for(ohel2=0;ohel2<2;++ohel2) {
 	  for(ohel3=0;ohel3<2;++ohel3) {
	    ...
	    me1(ihel1,ihel2,2*ohel1,ohel2,ohel3) = diag1 = _theFFWVertex->evaluate(_mw2,fin[ihel1],interb,
					      bcurr[ohel2][ohel3]);
	    me2(ihel1,ihel2,2*ohel1,ohel2,ohel3) = diag2 = _theFFWVertex->evaluate(_mw2,inters,ain[ihel2],
					      bcurr[ohel2][ohel3]);
	  // sum
	  newme(ihel1,ihel2,2*ohel1,ohel2,ohel3)= diag1+diag2;
	  }
	}
      }
    }
  ...
  // results
  // initial state colour average
  double col=1./9.;
  // and C_F N_c from matrix element
  col *= 4.;
  // colour factor for the W decay
  if(mePartonData()[3]->coloured()) col*=3.;
  if(!calc) {
    DVector save;
    save.push_back(me1.average(rhoin.first,rhoin.second)*col);
    save.push_back(me2.average(rhoin.first,rhoin.second)*col);
    meInfo(save);
  }
  // return the answer
  if(calc) _me.reset(newme);
  return newme.average(rhoin.first,rhoin.second)*col*UnitRemoval::InvE2;
}

InvEnergy2 MEPP2WJet::qgME(pair<RhoDMatrix,RhoDMatrix> & rhoin,
			   vector<SpinorWaveFunction> & fin,
			   vector<VectorWaveFunction> & gin,
			   vector<SpinorBarWaveFunction> & fout,
			   vector<SpinorBarWaveFunction> & lm,
  ...
  for(ihel1=0;ihel1<2;++ihel1) {
	...
	for(ohel2=0;ohel2<2;++ohel2) {
	  for(ohel3=0;ohel3<2;++ohel3) {
	    ...
	    me1(ihel1,2*ihel2,ohel1,ohel2,ohel3) = diag1 = _theFFWVertex->evaluate(_mw2,fin[ihel1],interb,
					    bcurr[ohel2][ohel3]);
	    me2(ihel1,2*ihel2,ohel1,ohel2,ohel3) = diag2 = _theFFWVertex->evaluate(_mw2,inters,fout[ohel1],
					    bcurr[ohel2][ohel3]);
	    // sum
	    newme(ihel1,2*ihel2,ohel1,ohel2,ohel3)= diag1+diag2;
	  }
	}
      }
    }
...
InvEnergy2 MEPP2WJet::qbargME(pair<RhoDMatrix,RhoDMatrix> & rhoin,
 ...
 ProductionMatrixElement newme(PDT::Spin1Half,PDT::Spin1,
  ...
  for(ihel1=0;ihel1<2;++ihel1) {
	...
	for(ohel2=0;ohel2<2;++ohel2) {
	  for(ohel3=0;ohel3<2;++ohel3) {
	    me1(ihel1,2*ihel2,ohel1,ohel2,ohel3) = diag1 = _theFFWVertex->evaluate(_mw2,inters,fin[ihel1],
					     bcurr[ohel2][ohel3]);
	    me2(ihel1,2*ihel2,ohel1,ohel2,ohel3) = diag2 = _theFFWVertex->evaluate(_mw2,fout[ohel1],interb,
					     bcurr[ohel2][ohel3]);
	    // sum
	    newme(ihel1,2*ihel2,ohel1,ohel2,ohel3)= diag1+diag2;
	  }
...
void MEPP2WJet::constructVertex(tSubProPtr sub) {
  ...
  if(hard[0]->id()<=6&&hard[0]->id()>0&&hard[1]->id()==ParticleID::g) {
    ...
    gin[1]=gin[2];
    qgME(rhoin,fin,gin,fout,lm,lp,true);
  }
  // qbar g to qbar W
    ...
    gin[1]=gin[2];
    qbargME(rhoin,ain,gin,aout,lm,lp,true);
  }
  // q qbar to g W
  else {
    vector<SpinorWaveFunction>     fin;
    ...
    gout[1]=gout[2];
    qqbarME(rhoin,fin,ain,gout,lm,lp,true);
  }
  hard[0]->spinInfo()->rhoMatrix(rhoin.first );
  hard[1]->spinInfo()->rhoMatrix(rhoin.second);
  if(hard[2]->id()==21) { //check for outgoing gluon
    RhoDMatrix temp(hard[2]->dataPtr()->iSpin());
    temp(0,0) = temp(2,2) = 0.5;
    temp(1,1)=0.;
    hard[2]->spinInfo()->DMatrix(temp);
  }
