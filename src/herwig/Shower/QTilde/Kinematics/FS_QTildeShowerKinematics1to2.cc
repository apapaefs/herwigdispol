...
updateChildren(const tShowerParticlePtr parent, 
  ...
  for(ShowerParticleVector::const_iterator pit=children.begin();
    ...
    if( (**pit).id()==21 || (**pit).id()==22 ) {
      (*pit)->spinInfo()->  DMatrix()(0,0) = (*pit)->spinInfo()->rhoMatrix()(0,0) = 0.5;
      (*pit)->spinInfo()->  DMatrix()(1,1) = (*pit)->spinInfo()->rhoMatrix()(1,1) = 0. ;
      (*pit)->spinInfo()->  DMatrix()(2,2) = (*pit)->spinInfo()->rhoMatrix()(2,2) = 0.5;
    }
  }
}
