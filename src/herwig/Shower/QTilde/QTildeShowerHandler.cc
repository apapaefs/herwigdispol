...
void QTildeShowerHandler::doinit() {
  ...
  _splittingGenerator->setPDFFreezingScale(pdfFreezingScale());
}
...
QTildeShowerHandler::spaceLikeShower(tShowerParticlePtr particle, PPtr beam,
  ...
  bool isPol = false;
  ThreeVector<double> pol;
  tcPDFPtr pdf,longpdf,transpdf;
  if(beam == incomingBeams().first) {
    pdf      = firstPDF().pdf();
    isPol    = isPolarized().first;
    pol      = polarization().first;
    longpdf  = longitudinalPDF().first;
    transpdf = transversePDF().first;
  }
  if(beam == incomingBeams().second) {
    pdf = secondPDF().pdf();
    isPol    = isPolarized().second;
    pol      = polarization().second;
    longpdf  = longitudinalPDF().second;
    transpdf = transversePDF().second;
  }
  // don't do anything if not needed
  ...
  while (true) {
    bb=_splittingGenerator->chooseBackwardBranching(*particle,beam,
						    _initialenhance,
						    _beam,type,
						    pdf,isPol,pol,longpdf,transpdf);
  ...
  if(!emitted) {
    if(_intrinsic.find(_progenitor)==_intrinsic.end()) {
      bb.kinematics->updateLast(beam->dataPtr(),newParent,ZERO,ZERO);
    }
    else {
      pair<Energy,double> kt=_intrinsic[_progenitor];
      bb.kinematics->updateLast(beam->dataPtr(),newParent,
  ...
  if(z>=0.&&z<=1.) {
    while (true) {
      ...
      bb = splittingGenerator()->chooseBackwardBranching( *particle,
							  beam, 1., beamParticle(),
							  type , pdf, isPol, pol, longpdf,transpdf);
  ...
  if( !bb.kinematics ) {
    ...
    if( !emitted ) {
      if( intrinsicpT().find( progenitor() ) == intrinsicpT().end() ) {
    	kinematics->updateLast(beam->dataPtr(), newParent, ZERO, ZERO );
      }
      else {
    	pair<Energy,double> kt = intrinsicpT()[progenitor()];
    	kinematics->updateLast( beam->dataPtr(), newParent,
    				kt.first*cos( kt.second ),
    				kt.first*sin( kt.second ) );
      }
    }
