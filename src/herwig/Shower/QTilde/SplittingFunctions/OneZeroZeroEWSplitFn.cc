...
DescribeClass<OneZeroZeroEWSplitFn,Sudakov1to2FormFactor>
...
OneZeroZeroEWSplitFn::forwardPhiWeights(const double, const Energy2, const IdList & ,
				       const RhoDMatrix &) {
...
vector<pair<int, Complex> >
OneZeroZeroEWSplitFn::backwardPhiWeights(const double, const Energy2, const IdList & ,
                                          const RhoDMatrix &) {
  // no dependence on the spin density matrix, dependence on off-diagonal terms cancels
  // and rest = splitting function for Tr(rho)=1 as required by defn
  return vector<pair<int, Complex> >(1,make_pair(0,1.));
...
vector<pair<int, Complex> >
OneZeroZeroEWSplitFn::backwardPhiWeights(const double, const Energy2, const IdList & ,
                                          const RhoDMatrix &, const RhoDMatrix &) {
  assert(false);
  // no dependence on the spin density matrix, dependence on off-diagonal terms cancels
  // and rest = splitting function for Tr(rho)=1 as required by defn
  return vector<pair<int, Complex> >(1,make_pair(0,1.));
