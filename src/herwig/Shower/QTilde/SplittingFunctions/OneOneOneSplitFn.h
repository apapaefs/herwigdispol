...
public:
  ...
  forwardPhiWeights(const double z, const Energy2, const IdList &,
  ...
  backwardPhiWeights(const double z, const Energy2, const IdList &,
    ...
    assert(rho.iSpin()==PDT::Spin1 && H.iSpin()==PDT::Spin1);
    double denom = sqr(1.-z*(1.-z));
    double diag = 1. + 2.*sqr(z)*real(rho(0,2)*H(0,2) + rho(2,0)*H(2,0))/denom + 
      z*(2 - z - 2*(1 - z)*z)*real((rho(2,2)-rho(0,0))*(H(2,2)-H(0,0)))/denom;
    Complex off  = -sqr(1.-z)*rho(2,0)/denom - sqr(z*(1.-z))*H(0,2)/denom;
    double max  = diag+2*abs(off);
    vector<pair<int, Complex> > output;
    output.push_back(make_pair( 0, diag/max));
    output.push_back(make_pair( 2, conj(off)/max));
    output.push_back(make_pair(-2, off/max));
    return output;
  }
  ...
  virtual double initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const {
    double den = sqr(1 - (1 - z)*z);
    return 1. + (2.*sqr(z)*(rho(0,2)*H(0,2) + rho(2,0)*H(2,0))/den + z*(2. - z - 2.*(1 - z)*z)*(rho(2,2)-rho(0,0))*(H(2,2)-H(0,0))/den).real();
  }
