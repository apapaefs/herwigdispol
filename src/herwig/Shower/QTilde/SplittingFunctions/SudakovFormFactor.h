...
public:
  ...
  virtual double polarizationVetoRatio(const Energy2 t, const IdList &ids,
                                       const RhoDMatrix & H, const RhoDMatrix & rho) = 0;
   ...
   * @param The Shower kinematics
   */
  virtual double generatePhiBackward(ShowerParticle & particle,
                                     Ptr<BeamParticleData>::transient_const_pointer beam,
                                     const IdList & ids, ShoKinPtr kinematics,
                                     const RhoDMatrix & rho, double x)=0;
...
public:
  ...
  /**
   *   Caclulation of the H matrix for polarized partons
   */
...
protected:
  ...
  bool PDFVeto(const Energy2 t, const double x, const double z,
	       ...
	       tcBeamPtr beam, const RhoDMatrix & rho) const;
  /**
   * The PDF veto ratio
   */
  double PDFVetoRatio(const Energy2 t, const double x, const double z,
                      const tcPDPtr parton0, const tcPDPtr parton1,
                      tcBeamPtr beam, const RhoDMatrix & rho, double factor) const;

  /**
   *   Set the PDF
   */
  void setPDF(tcPDFPtr pdf, Energy scale, bool isPol,
              ThreeVector<double> pol, tcPDFPtr longPDF, tcPDFPtr transPDF) {
    pdf_ = pdf;
    freeze_ = scale;
    isPol_ = isPol;
    polarization_ = pol;
  ...
  /**
   *   Polarization weight for ISR
   */
  virtual double initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const;

  /**
   *   Whether or not incoming hadron is polarized
   */
  bool isPolarized() const {return isPol_;}
...
private:
  ...
  /**
   *  Whether or not beams are polarized
   */
  bool isPol_;
   ...
   * Beam polarizations
   */
  ThreeVector<double> polarization_;

  /**
   *   Longitudinal difference pdfs
   */
  tcPDFPtr longPDF_;

  /**
   *   Transverse difference pdfs
   */
  tcPDFPtr transPDF_;
