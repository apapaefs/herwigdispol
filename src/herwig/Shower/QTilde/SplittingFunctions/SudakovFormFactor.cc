...
bool SudakovFormFactor::PDFVeto(const Energy2 t, const double x, const double z,
                                ...
                                Ptr<BeamParticleData>::transient_const_pointer beam, const RhoDMatrix & rho) const {
  double ratio=PDFVetoRatio(t,x,z,parton0,parton1,beam,rho,1.);
...
double SudakovFormFactor::PDFVetoRatio(const Energy2 t, const double x, const double z,
  ...
  // finally for polarized hadrons
  if( isPol_ ) {
    // first compute the H matrices before and after emission
    unsigned int imaxOld = parton1->iSpin()-1;
    unsigned int imaxNew = parton0->iSpin()-1;
    RhoDMatrix HOld(parton1->iSpin()),HNew(parton0->iSpin());
    if(parton0->id()==21 || parton0->id()==22) {
      HNew(0,0) = 0.5;
      HNew(1,1) = 0.0;
      HNew(2,2) = 0.5;
    }
    if(parton1->id()==21 || parton1->id()==22) {
      HOld(0,0) = 0.5;
      HOld(1,1) = 0.0;
      HOld(2,2) = 0.5;
    }
    double pL = polarization_.z();
    Complex pT(polarization_.x(),polarization_.y());
    double longOld=0., longNew=0.;
    // longitudinal polarization
    if (  polarization_.z() != 0. && longPDF_) {
      longOld = max(-1.,min(1.,longPDF_->xfl(beam,parton1,theScale,x  )/oldpdf));
      longNew = max(-1.,min(1.,longPDF_->xfl(beam,parton0,theScale,x/z)/newpdf));
    }
    double transOld(0.),transNew(0.);
    // transverse polarization
    if( (polarization_.x() !=0. || polarization_.y() !=0. ) && transPDF_) {
      transOld = transPDF_->xfl(beam,parton1,theScale,x  )/oldpdf;
      transNew = transPDF_->xfl(beam,parton0,theScale,x/z)/newpdf;
    }
    HOld(0      ,0      ) = 0.5*(1.-pL*longOld);
    HOld(imaxOld,imaxOld) = 0.5*(1.+pL*longOld);
    HOld(0      ,imaxOld) = 0.5*pT*transOld;
    HOld(imaxOld,0      ) = 0.5*conj(pT)*transOld;
    HNew(0      ,0      ) = 0.5*(1.-pL*longNew);
    HNew(imaxNew,imaxNew) = 0.5*(1.+pL*longNew);
    HNew(0      ,imaxNew) = 0.5*pT*transNew;
    HNew(imaxNew,0      ) = 0.5*conj(pT)*transNew;
    // denominator of weight
    Complex den = 0.;
    unsigned int ispin = parton1->iSpin();
    for(unsigned int ix=0;ix<ispin;++ix) 
...
double SudakovFormFactor::initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const {
  cerr << "\n" << " " << "\n" << fullName() << "\n";
  assert(false);
}

RhoDMatrix SudakovFormFactor::calculateHMatrix(tcPDPtr beam,
                                               const tcPDPtr parton, double x, Energy2 scale) {
  RhoDMatrix H(parton->iSpin());
  if(parton->id()==21 || parton->id()==22) {
    H(0,0) = 0.5;
    H(1,1) = 0.0;
    H(2,2) = 0.5;
  }
  if(!isPol_ || !pdf_) return H;
  const double oldpdf=pdf_->xfx(beam,parton,scale,x);
  if(oldpdf<=0.) return H;
  unsigned int imax = parton->iSpin()-1;
  // longitudinal polarization
  double pL = polarization_.z();
  if (  polarization_.z() != 0. && longPDF_)
    pL *= max(-1.,min(1.,longPDF_->xfl(beam,parton,scale,x  )/oldpdf));
  // transverse polarization
  Complex pT(polarization_.x(),polarization_.y());
  if( (polarization_.x() !=0. || polarization_.y() !=0. ) && transPDF_)
    pT *= transPDF_->xfl(beam,parton,scale,x)/oldpdf;
  H(0   ,0   ) = 0.5*(1.-pL);
  H(imax,imax) = 0.5*(1.+pL);
  H(0   ,imax) = 0.5*pT;
  H(imax,0   ) = 0.5*conj(pT);
  return H;
}
