...
public:
   ...
   */
  virtual vector<pair<int,Complex> > 
  backwardPhiWeights(const double z, const Energy2 t, const IdList & ids,
		      const RhoDMatrix &);
  ...
  backwardPhiWeights(const double z, const Energy2, const IdList &,
    ...
    double denom = (2.-2.*z+sqr(z));
    double diag = 1 + (2.-z)*z*real((rho(2,2)-rho(0,0))*(H(1,1)-H(0,0)))/denom;
    Complex off  =-2.*(1.-z)*rho(2,0)/denom;
  ...
  virtual double initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const {
    return 1 + (2. - z)*z*(rho(2,2)-rho(0,0)).real()*(H(1,1)-H(0,0)).real()/(2. - 2.*z + sqr(z));
  }
