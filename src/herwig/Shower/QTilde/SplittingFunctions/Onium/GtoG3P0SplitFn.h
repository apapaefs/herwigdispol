// -*- C++ -*-
#ifndef Herwig_GtoG3P0SplitFn_H
#define Herwig_GtoG3P0SplitFn_H
//
// This is the declaration of the GtoG3P0SplitFn class.
//
...
/**
 * The GtoG3P0SplitFn class implements the colour singlet splitting function for \f$g\to g \chi_{0(c,b)}\f$.
 *
 * @see \ref GtoG3P0SplitFnInterfaces "The interfaces"
 * defined for GtoG3P0SplitFn.
 */
class GtoG3P0SplitFn: public Sudakov1to2FormFactor {
...
public:
   ...
   */
  GtoG3P0SplitFn() : O1_(0.573*GeV*GeV2*GeV2), state_(ccbar), n_(1), m_(1.2*GeV), fixedAlphaS_(-1.)
  {}
...
public:
  ...
  bool accept(const IdList & ids) const {
    ...
    // check onium state
    int iq=4+state_;
    long idtest = iq*110+10001 + (n_-1)*100000;
  ...
  double ratioP(const double z, const Energy2 t,
    ...
    double ratio = 1 - 2*(1-z)*z -8*r*(2 + z*(-3 + 2*z)) + 32*sqr(r)*(2 - 3*sqr(z)) -512*pow(r,3)*(1-z)*z + 2048*pow(r,4)*sqr(z);
    ratio /= pOver_;
    if(ratio>1.) cerr << "problem in GtoG3P0 ratio violated " << ratio << " " << pOver_ << "\n";
    return ratio;
  }
  ...
  forwardPhiWeights(const double z, const Energy2 t, const IdList &,
		     const RhoDMatrix & rho) {
    assert(rho.iSpin()==PDT::Spin1);
    double r = sqr(m_)/t;
    double on  =  1.+.16*sqr(r)-8.*r*(1.-z)-2.*(1.-z)*z;
    double off = -2*(1-4.*r-z)*z;
   ...
   */
  vector<pair<int,Complex> >
  backwardPhiWeights(const double, const Energy2, const IdList &,
		      const RhoDMatrix & rho) {
    assert(false);
    assert(rho.iSpin()==PDT::Spin1);
    vector<pair<int, Complex> > output;
   ...
   */
  vector<pair<int,Complex> > 
  backwardPhiWeights(const double z, const Energy2, const IdList &,
		      const RhoDMatrix & rho, const RhoDMatrix & H) {
    assert(false);
    return {{ {0, 1.} }};
  }
  ...
  DecayMEPtr matrixElement(const double z, const Energy2 t, 
    ...
    (*kernal)(0,0,0) = z;
    (*kernal)(2,2,0) = z;
    (*kernal)(0,2,0) = (1.-4.*r-z)/phase;
    (*kernal)(2,0,0) = (1.-4.*r-z)*phase;
    return kernal;
  }
...
private:
   ...
   */
  GtoG3P0SplitFn & operator=(const GtoG3P0SplitFn &) = delete;
