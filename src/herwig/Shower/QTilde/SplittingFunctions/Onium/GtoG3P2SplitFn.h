// -*- C++ -*-
#ifndef Herwig_GtoG3P2SplitFn_H
#define Herwig_GtoG3P2SplitFn_H
//
// This is the declaration of the GtoG3P2SplitFn class.
//
...
/**
 * The GtoG3P2SplitFn class implements the colour singlet splitting function for \f$g\to g \chi_{0(c,b)}\f$.
 *
 * @see \ref GtoG3P2SplitFnInterfaces "The interfaces"
 * defined for GtoG3P2SplitFn.
 */
class GtoG3P2SplitFn: public Sudakov1to2FormFactor {
...
public:
   ...
   */
  GtoG3P2SplitFn() : O1_(0.573*GeV*GeV2*GeV2), state_(ccbar), n_(1), m_(1.2*GeV), fixedAlphaS_(-1.)
  {}
...
public:
  ...
  bool accept(const IdList & ids) const {
    ...
    // check onium state
    int iq=4+state_;
    long idtest = iq*110+5 + (n_-1)*100000;
  ...
  double ratioP(const double z, const Energy2 t,
    ...
    double ratio = 1 - 2*(1.-z)*z + 8*r*(1 + (3 - 2*z)*z) + 16*sqr(r)*(7 + 6*(1 - 2*z)*z) - 128*pow(r,3)*z*(1 + 2*z) + 512*pow(r,4)*sqr(z);
    ratio /= pOver_;
    if(ratio>1.) cerr << "problem in GtoG3P2 ratio violated " << ratio << " " << pOver_ << "\n";
    return ratio;
  }
...
private:
   ...
   */
  GtoG3P2SplitFn & operator=(const GtoG3P2SplitFn &) = delete;
