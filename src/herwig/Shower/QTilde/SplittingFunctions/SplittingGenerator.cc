...
chooseBackwardBranching(ShowerParticle &particle,PPtr,
			...
			tcPDFPtr pdf, bool isPol, ThreeVector<double> pol,
                        tcPDFPtr longPDF, tcPDFPtr transPDF) const {
  RhoDMatrix rho;
  bool rhoCalc(false);
  Energy newQ=ZERO;
  ShoKinPtr kinematics=ShoKinPtr();
  ...
  for(BranchingList::const_iterator cit = _bbranchings.lower_bound(index);
    ...
    if(!checkInteraction(type,cit->second.sudakov->interactionType(),_darkInteraction)) continue;
    // setup the PDF
    cit->second.sudakov->setPDF(pdf,_freeze,isPol,pol,longPDF,transPDF);
    //calc rho as needed
    if(!rhoCalc && particle.dataPtr()->iSpin()!=PDT::Spin0) {
  ...
  // and generate phi
  kinematics->phi(sudakov->generatePhiBackward(particle,beam,ids,kinematics,rho,particle.x()));
  // return the answer
  return Branching(kinematics, ids,sudakov,partnerType);
}
