...
public:
   ...
   * @param The Shower kinematics
   */
  double generatePhiBackward(ShowerParticle & particle,
                             Ptr<BeamParticleData>::transient_const_pointer beam,
                             const IdList & ids, ShoKinPtr kinematics,
                             const RhoDMatrix & rho, double x);
...
public:
   ...
   * should be simple enough that it does not depend on the evolution scale.const Energy2 t, const double x, const double z,
                      const tcPDPtr parton0, const tcPDPtr parton1,
                      tcBeamPtr beam,double factor
   * @param z   The energy fraction.
   * @param ids The PDG codes for the particles in the splitting.
   */
   ...
   */
  virtual vector<pair<int,Complex> > 
  backwardPhiWeights(const double, const Energy2, const IdList & , const RhoDMatrix &, const RhoDMatrix &) = 0;
...
protected:
  ...
  virtual double polarizationVetoRatio(const Energy2 t, const IdList &ids,
                                       const RhoDMatrix & H, const RhoDMatrix & rho);
