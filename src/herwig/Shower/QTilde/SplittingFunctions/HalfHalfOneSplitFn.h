...
public:
  ...
  forwardPhiWeights(const double, const Energy2, const IdList & , const RhoDMatrix &) {
   ...
   */
  vector<pair<int,Complex> > 
  backwardPhiWeights(const double, const Energy2, const IdList & , const RhoDMatrix &) {
    // no dependence on the spin density matrix, dependence on off-diagonal terms cancels
    // and rest = splitting function for Tr(rho)=1 as required by defn
    return {{ {0, 1.} }};
  }
   ...
   */
  vector<pair<int,Complex> > 
  backwardPhiWeights(const double, const Energy2, const IdList & , const RhoDMatrix &, const RhoDMatrix &) {
    // no dependence on the spin density matrix, dependence on off-diagonal terms cancels
    // and rest = splitting function for Tr(rho)=1 as required by defn
    return {{ {0, 1.} }};
  }
  ...
  virtual double initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const {
    return 1. + 4*z*(rho(0,1)*H(0,1)+rho(1,0)*H(1,0)).real()/(1. + sqr(z)) + (rho(1,1)-rho(0,0)).real()*(H(1,1)-H(0,0)).real();
  }
