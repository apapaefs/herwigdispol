...
public:
  ...
  forwardPhiWeights(const double z, const Energy2 t, const IdList & ids,
	      const RhoDMatrix &);
  ...
  backwardPhiWeights(const double z, const Energy2, const IdList &,
    ...
    double denom = 1.-2.*(1.-z)*z;
    Complex off = 2.*(1.-z)*z*H(0,2)/denom;
    double diag = 1.-(1.-2.*z)*real((rho(1,1)-rho(0,0))*(H(2,2)-H(0,0)))/denom;
    double max = diag+2.*abs(off);
    vector<pair<int, Complex> > output;
    output.push_back(make_pair( 0, (rho(0,0)+rho(1,1))*diag/max));
    output.push_back(make_pair( 2,-rho(0,1)           * off/max));
    output.push_back(make_pair(-2,-rho(1,0)           * off/max));
    return output;
  }
  ...
  virtual double initialStatePolarizationWeight(const double & z, const RhoDMatrix & H, const RhoDMatrix & rho) const {
    return 1. - (1.-2.*z)/(1.-2*(1.-z)*z)*(rho(1,1) - rho(0,0)).real()*(H(2,2)-H(0,0)).real();
  }
