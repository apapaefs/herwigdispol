...
generateNextSpaceBranching(const Energy startingQ,
  ...
  if(!ShowerHandler::currentHandlerIsSet() ||
    ...
    while(pt2 < cutoff_->pT2min()||
          ...
          PDFVeto(t,x,z(),ids[0],ids[1],beam,rho));
  }
  // shower variations
  else {
    ...
    do {
      ...
      PDFRew=PDFVeto(t,x,z(),ids[0],ids[1],beam,rho);
      double factor=PDFVetoRatio(t,x,z(),ids[0],ids[1],beam,rho,1.)*
	...
	SplittingFnVetoRatio((1.-z())*t/z(),ids,false,rho,detuning);
      ...
      for ( map<string,ShowerVariation>::const_iterator var =
        ...
        if ( ( ch->firstInteraction() && var->second.firstInteraction ) ||
             ( !ch->firstInteraction() && var->second.secondaryInteractions ) ) {
          double newfactor = PDFVetoRatio(t,x,z(),ids[0],ids[1],beam,rho,var->second.factorizationScaleFactor)*
...
double Sudakov1to2FormFactor::generatePhiForward(ShowerParticle & particle,
  ...
  if(!ShowerHandler::currentHandlerIsSet() ||
    ...
    wgts = forwardPhiWeights(z,t,ids,rho);
  }
  else {
    wgts = {{ {0, 1.} }};
  }
...
double Sudakov1to2FormFactor::polarizationVetoRatio(const Energy2 t, const IdList &ids,
