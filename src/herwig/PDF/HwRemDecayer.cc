...
PPtr HwRemDecayer::forceSplit(const tRemPPtr rem, long child, Energy &lastQ, 
  ...
  if((!theRems.first || !theRems.second) &&
     abs(child) >= ParticleID::c && abs(child) <= ParticleID::t) {
    // Note: this change was made for DIS remnant recovery. The forced
    // backward-splitting step uses massless kernels, and assigning a
    // constituent heavy-quark mass here can overconstrain the later
    // remnant reshuffle for sea-charm events.
    emittedm2 = ZERO;
  }
...
void HwRemDecayer::setRemMasses(PPair diquarks) const {
  ...
  if(!theprocessed[0]||!theprocessed[1]) {
    ...
    Energy effectiveDiquarkMass(diquark->mass());
	...
	continue;
      ...
      // not connected
      // Keep the full final-state branch as a recoil fallback. In DIS this is
      // what allows the scattered lepton itself to participate when it is a
      // leaf particle, rather than silently dropping it from the "use the
      // lepton if needed" path below.
      findChildren(*cit,others);
    }
    ...
    do {
      ...
      psystem.rescaleMass();
      ++iloc;
      if(ptotal.mass() > psystem.mass() + effectiveDiquarkMass &&
	 psystem.mass()>1*MeV && DISRemnantOpt_<2 && ptotal.e() > 0.*GeV ) break;
    }
    while(iloc<progenitors.size());
    if(ptotal.mass() > psystem.mass() + effectiveDiquarkMass) --iloc;
	...
	if(DISRemnantOpt_==0||DISRemnantOpt_==2)
      ...
      else if((DISRemnantOpt_==0||DISRemnantOpt_==2) &&
	      DISOneRemnantFallbackOpt_ == 1 &&
	      psystem.mass()>1*MeV && ptotal.e() > 0.*GeV &&
	      ptotal.mass() >= psystem.mass()) {
	// Note: this opt-in recovery was added for fixed-order-style DIS runs
	// where the remnant is not an observable. If the selected recoil
	// system is still insufficient, prefer a kinematically allowed
	// effective remnant mass over vetoing the whole event.
	Energy availableRemnantMass = ptotal.mass() - psystem.mass();
	effectiveDiquarkMass = availableRemnantMass;
	if(effectiveDiquarkMass > diquark->mass())
	  effectiveDiquarkMass = diquark->mass();
	if(effectiveDiquarkMass > ZERO &&
	   ptotal.mass() <= psystem.mass() + effectiveDiquarkMass)
	  effectiveDiquarkMass *= 0.999999;
	if(ptotal.mass() >= psystem.mass() + effectiveDiquarkMass) {
	  Throw<Exception>() << "Warning relaxed the one-remnant DIS mass"
			     << " constraint in HwRemDecayer::setRemMasses()"
			     << " to avoid vetoing an event"
			     << Exception::warning;
	}
	else {
    ...
    Energy pz = SimplePhaseSpace::getMagnitude(sqr(ptotal.mass()), 
					       psystem.mass(), effectiveDiquarkMass);
    LorentzRotation Rs(-(R*psystem).boostVector());
    Rs.boost(0.0, 0.0, pz/sqrt(sqr(pz) + sqr(psystem.mass())));
    Rs = Rs*R;
    // put remnant on shell
    deltap.transform(R);
    deltap.setMass(effectiveDiquarkMass);
    deltap.setE(sqrt(sqr(effectiveDiquarkMass)+sqr(pz)));
    deltap.rescaleRho();
    R.invert();
    deltap.transform(R);
    Rs = R*Rs;
...
void HwRemDecayer::Init() {
  ...
  static Switch<HwRemDecayer,unsigned int> interfaceDISOneRemnantFallback
    ("DISOneRemnantFallback",
     "Optional recovery mode for one-remnant DIS configurations when the"
     " nominal remnant mass would otherwise veto the event.",
     &HwRemDecayer::DISOneRemnantFallbackOpt_, 0, false, false);
  static SwitchOption interfaceDISOneRemnantFallbackDefault
    (interfaceDISOneRemnantFallback,
     "Default",
     "Keep the nominal one-remnant DIS mass requirement.",
     0);
  static SwitchOption interfaceDISOneRemnantFallbackRelaxMass
    (interfaceDISOneRemnantFallback,
     "RelaxMass",
     "As a last resort, reduce the effective remnant mass to the"
     " largest kinematically allowed value.",
     1);
