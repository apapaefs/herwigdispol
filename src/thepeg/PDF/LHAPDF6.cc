...
double ThePEG::LHAPDF::alphaS(Energy2 partonScale) const {
  const_cast<ThePEG::LHAPDF *>(this)->initPDFptr();
  double Q2 = partonScale/GeV2;

  if ( !thePDF->inRangeXQ2(0.5, Q2) ) {
    switch ( rangeException ) {
    case rangeThrow:
      Throw<Exception>()
        << "Scale (Q2=" << Q2 << " GeV^2) was outside of limits in PDF "
